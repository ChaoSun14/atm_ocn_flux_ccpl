...
!--------------------------------------------------------------------------
  ...
  !edited by Sun Chao
  public use_ccpl_do_flux_atmocn_init
  public use_ccpl_do_flux_atmocn_run
...
!--------------------------------------------------------------------------
  ...
  !edited by Sun Chao 
  type, private :: ccpl_flux_var 
    integer               :: ccpl_flux_inst_id
    integer               :: ccpl_flux_timer_id
    integer,  allocatable :: ccpl_flux_fields_id(:)
  ...
  end type ccpl_flux_var
  type(ccpl_flux_var) :: ccpl_flux_atmocn
...
subroutine use_ccpl_do_flux_atmocn_init(cdata, a2x, o2x, xao, comp_id, grid_id, decomp_id, annotation)
    implicit none
    type(seq_cdata), intent(in)    :: cdata
    type(mct_aVect), intent(in)    :: a2x  
    type(mct_aVect), intent(in)    :: o2x
    type(mct_aVect), intent(inout) :: xao
    integer,         intent(in)    :: comp_id, grid_id, decomp_id
    character(*),    intent(in), optional :: annotation
    ...
    logical     :: dead_comps   ! .true.  => dead components are used
    logical     :: first_call = .true.
    integer     :: field_id_mask
    character(*) :: local_annot
    ...
    local_annot = ""
    if (present(annotation)) local_annot = trim(annotation)
    
    field_id_mask = CCPL_register_field_instance(mask, "mask", decomp_id, grid_id, 37, annotation=local_annot)

    allocate(ccpl_flux_atmocn%ccpl_flux_fields_id(24))
    ccpl_flux_atmocn%ccpl_flux_fields_id(1)  = xao%field_ids(index_xao_So_tref)
    ccpl_flux_atmocn%ccpl_flux_fields_id(2)  = xao%field_ids(index_xao_So_qref)
    ccpl_flux_atmocn%ccpl_flux_fields_id(3)  = xao%field_ids(index_xao_So_ustar)
    ccpl_flux_atmocn%ccpl_flux_fields_id(4)  = xao%field_ids(index_xao_So_re)
    ccpl_flux_atmocn%ccpl_flux_fields_id(5)  = xao%field_ids(index_xao_So_ssq)
    ccpl_flux_atmocn%ccpl_flux_fields_id(6)  = xao%field_ids(index_xao_So_u10)
    ccpl_flux_atmocn%ccpl_flux_fields_id(7)  = xao%field_ids(index_xao_So_duu10n)
    ccpl_flux_atmocn%ccpl_flux_fields_id(8)  = xao%field_ids(index_xao_Faox_taux)
    ccpl_flux_atmocn%ccpl_flux_fields_id(9)  = xao%field_ids(index_xao_Faox_tauy)
    ccpl_flux_atmocn%ccpl_flux_fields_id(10) = xao%field_ids(index_xao_Faox_lat)
    ccpl_flux_atmocn%ccpl_flux_fields_id(11) = xao%field_ids(index_xao_Faox_sen)
    ccpl_flux_atmocn%ccpl_flux_fields_id(12) = xao%field_ids(index_xao_Faox_evap)
    ccpl_flux_atmocn%ccpl_flux_fields_id(13) = xao%field_ids(index_xao_Faox_lwup)
    ccpl_flux_atmocn%ccpl_flux_fields_id(14) = a2x%field_ids(index_a2x_Sa_z)
    ccpl_flux_atmocn%ccpl_flux_fields_id(15) = a2x%field_ids(index_a2x_Sa_u)
    ccpl_flux_atmocn%ccpl_flux_fields_id(16) = a2x%field_ids(index_a2x_Sa_v)
    ccpl_flux_atmocn%ccpl_flux_fields_id(17) = a2x%field_ids(index_a2x_Sa_tbot)
    ccpl_flux_atmocn%ccpl_flux_fields_id(18) = a2x%field_ids(index_a2x_Sa_ptem)
    ccpl_flux_atmocn%ccpl_flux_fields_id(19) = a2x%field_ids(index_a2x_Sa_shum)
    ccpl_flux_atmocn%ccpl_flux_fields_id(20) = a2x%field_ids(index_a2x_Sa_dens)
    ccpl_flux_atmocn%ccpl_flux_fields_id(21) = o2x%field_ids(index_o2x_So_t)
    ccpl_flux_atmocn%ccpl_flux_fields_id(22) = o2x%field_ids(index_o2x_So_u)
    ccpl_flux_atmocn%ccpl_flux_fields_id(23) = o2x%field_ids(index_o2x_So_v)
    ccpl_flux_atmocn%ccpl_flux_fields_id(24) = field_id_mask

    allocate(ccpl_flux_atmocn%control_vars(2))
    if (dead_comps) then 
      ccpl_flux_atmocn%control_vars(1) = 1
    else
      ccpl_flux_atmocn%control_vars(1) = 0
    end if
    ccpl_flux_atmocn%control_vars(2) = nloc
    allocate(ccpl_flux_atmocn%comp_or_grid_ids(1))
    allocate(ccpl_flux_atmocn%decomp_ids(1))
    ccpl_flux_atmocn%comp_or_grid_ids(1) = grid_id
    ccpl_flux_atmocn%decomp_ids(1) = decomp_id
   
    ccpl_flux_atmocn%ccpl_flux_inst_id = CCPL_external_modules_inst_init(comp_id, "ccpl_flux_atmocn", & 
        "atmocn_flux", "individual","libatmocn_flux.so", .true., ccpl_flux_atmocn%control_vars, ccpl_flux_atmocn%comp_or_grid_ids, &
        ccpl_flux_atmocn%decomp_ids, ccpl_flux_atmocn%ccpl_flux_fields_id, annotation="do external procedure atmocn_flux initialize")

end subroutine use_ccpl_do_flux_atmocn_init

subroutine use_ccpl_do_flux_atmocn_run(a2x, o2x, xao)
    implicit none
    type(mct_aVect), intent(in)    :: a2x  
    type(mct_aVect), intent(in)    :: o2x
    type(mct_aVect), intent(inout) :: xao

    call mct_avect_export(a2x,'Sa_z:Sa_u:Sa_v:Sa_tbot:Sa_ptem:Sa_shum:Sa_dens')
    call mct_avect_export(o2x,'So_t:So_u:So_v')
    
    call CCPL_external_modules_inst_run(ccpl_flux_atmocn%ccpl_flux_inst_id,-1,annotation="do external procedure atmocn_flux run")

    call mct_avect_import(xao,'So_tref:So_qref:So_ustar:So_re:So_ssq:So_u10:So_duu10n:Faox_taux:Faox_tauy:Faox_lat:Faox_sen:Faox_evap:Faox_lwup')

end subroutine use_ccpl_do_flux_atmocn_run
