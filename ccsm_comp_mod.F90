...
!  character(CL) :: hist_a2x24hr_flds = 'all'
   ...
   !edited by Zhang Cheng
#define USE_C_COUPLER
!edited by Sun Chao
#define CCPL_FLUX
#define BINARY_CHECK_FLUX

#ifdef USE_C_COUPLER
!#define BINARY_CHECK
   logical       :: use_ccpl = .true.
   logical       :: use_trans = .true.
   logical       :: ltemp
   integer       :: itemp
   character(CS) :: ctemp

   integer  :: xao_timer_id
   integer  :: xao_comp_id
   integer  :: xaoatm_grid_id
   integer  :: xaoocn_grid_id
   integer  :: xaoatm_decomp_id
   integer  :: xaoocn_decomp_id

   integer  :: dtimel                 ! dt of one lnd coupling interval
   integer  :: dtimei                 ! dt of one ice coupling interval
   integer  :: dtimeg                 ! dt of one glc coupling interval
   integer  :: dtimew                 ! dt of one wav coupling interval

   integer  :: cpl_timer_id
   integer  :: cplatm_timer_id(num_inst_atm)
   integer  :: cpllnd_timer_id(num_inst_lnd)
   integer  :: cplice_timer_id(num_inst_ice)
   integer  :: cplocn_Cx2o_timer_id(num_inst_ocn)
   integer  :: cplocn_Co2x_timer_id(num_inst_ocn)
   integer  :: cplglc_timer_id(num_inst_glc)
   integer  :: cplrof_timer_id(num_inst_rof)
   integer  :: cplwav_timer_id(num_inst_wav)
   integer  :: atm_timer_id(num_inst_atm)
   integer  :: lnd_timer_id(num_inst_lnd)
   integer  :: ice_timer_id(num_inst_ice)
   integer  :: ocn_Cx2o_timer_id(num_inst_ocn)
   integer  :: ocn_Co2x_timer_id(num_inst_ocn)
   integer  :: glc_timer_id(num_inst_glc)
   integer  :: rof_timer_id(num_inst_rof)
   integer  :: wav_timer_id(num_inst_wav)
   !integer  :: sno_timer_id(num_inst_atm)

   integer  :: root_comp_id
   integer  :: cpl_comp_id
   integer  :: atm_comp_id(num_inst_atm)           ! MPI atm communicator
   integer  :: lnd_comp_id(num_inst_lnd)           ! MPI lnd communicator
   integer  :: ice_comp_id(num_inst_ice)           ! MPI ice communicator
   integer  :: ocn_comp_id(num_inst_ocn)           ! MPI ocn communicator
   integer  :: glc_comp_id(num_inst_glc)           ! MPI glc communicator
   integer  :: rof_comp_id(num_inst_rof)           ! MPI rof communicator
   integer  :: wav_comp_id(num_inst_wav)           ! MPI wav communicator
   !integer  :: sno_comp_id(num_inst_lnd)           ! MPI sno communicator

   integer  :: atm_grid_id(num_inst_atm)
   integer  :: lnd_grid_id(num_inst_lnd)
   integer  :: ice_grid_id(num_inst_ice)
   integer  :: ocn_grid_id(num_inst_ocn)
   integer  :: glc_grid_id(num_inst_glc)
   integer  :: rof_grid_id(num_inst_rof)
   integer  :: wav_grid_id(num_inst_wav)
   integer  :: sno_grid_id(num_inst_lnd)

   integer  :: cplatm_grid_id
   integer  :: cpllnd_grid_id
   integer  :: cplice_grid_id
   integer  :: cplocn_grid_id
   integer  :: cplglc_grid_id
   integer  :: cplrof_grid_id
   integer  :: cplwav_grid_id
   integer  :: cplsno_grid_id

   integer  :: atm_decomp_id(num_inst_atm)
   integer  :: lnd_decomp_id(num_inst_lnd)
   integer  :: ice_decomp_id(num_inst_ice)
   integer  :: ocn_decomp_id(num_inst_ocn)
   integer  :: glc_decomp_id(num_inst_glc)
   integer  :: rof_decomp_id(num_inst_rof)
   integer  :: wav_decomp_id(num_inst_wav)
   integer  :: sno_decomp_id(num_inst_lnd)

   integer  :: cplatm_decomp_id
   integer  :: cpllnd_decomp_id
   integer  :: cplice_decomp_id
   integer  :: cplocn_decomp_id
   integer  :: cplglc_decomp_id
   integer  :: cplrof_decomp_id
   integer  :: cplwav_decomp_id
   integer  :: cplsno_decomp_id

   character(CL)  :: comps_full_names(12) 
#else
   logical  :: use_ccpl=.false.
   logical  :: use_trans=.false.
#endif
...
!   endif
   ...
   !edited by Zhang Cheng
#ifdef USE_C_COUPLER
   call use_ccpl_register_component()
#endif
...
#ifdef USE_C_COUPLER
            call use_ccpl_register_atm_grid(eai)
#endif
            call mct_avect_zero(x2a_aa(eai))
            call seq_map_map(mapper_Ca2x(eai), x2a_aa(eai), x2a_ax(eai), msgtag=CPLATMID(eai)*100+eai*10+3, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Ca2x(eai), x2a_aa(eai), x2a_ax(eai), msgtag=CPLATMID(eai)*100+eai*10+3)
            if (iamin_CPLID) then
                ltemp = seq_map_check(x2a_ax(eai))
                !if (.not. ltemp) write(*,*) "MMMMMM init mapper_Ca2x: ", ltemp
                write(*,*) "MMMMMM init mapper_Ca2x: ", ltemp
            end if
#endif
        endif
     enddo
...
#ifdef USE_C_COUPLER
            call use_ccpl_register_lnd_grid(eli)
#endif
            call mct_avect_zero(x2l_ll(eli))
            call seq_map_map(mapper_Cl2x(eli), x2l_ll(eli), x2l_lx(eli), msgtag=CPLLNDID(eli)*100+eli*10+3, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Cl2x(eli), x2l_ll(eli), x2l_lx(eli), msgtag=CPLLNDID(eli)*100+eli*10+3)
            if (iamin_CPLID) then
                ltemp = seq_map_check(x2l_lx(eli))
                if(.not. ltemp) write(*,*) "MMMMMM init mapper_Cl2x: ", ltemp
            end if
#endif
         endif
      enddo
...
#ifdef USE_C_COUPLER
            call use_ccpl_register_sno_grid(eli)
#endif
...
#ifdef USE_C_COUPLER
            call use_ccpl_register_rof_grid(eri)
#endif
            call mct_avect_zero(x2r_rr(eri))
            call seq_map_map(mapper_Cr2x(eri), x2r_rr(eri), x2r_rx(eri), msgtag=CPLROFID(eri)*100+eri*10+3, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Cr2x(eri), x2r_rr(eri), x2r_rx(eri), msgtag=CPLROFID(eri)*100+eri*10+3)
            if (iamin_CPLID) then
                ltemp = seq_map_check(x2r_rx(eri))
                if (.not. ltemp) write(*,*) "MMMMMM init mapper_Cr2x: ", ltemp
            end if
#endif
         endif
      enddo
...
#ifdef USE_C_COUPLER
            call use_ccpl_register_ocn_grid(eoi)
#endif
            call mct_avect_zero(x2o_oo(eoi))
            call seq_map_map(mapper_Co2x(eoi), x2o_oo(eoi), x2o_ox(eoi), msgtag=CPLOCNID(eoi)*100+eoi*10+3, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Co2x(eoi), x2o_oo(eoi), x2o_ox(eoi), msgtag=CPLOCNID(eoi)*100+eoi*10+3)
            if (iamin_CPLID) then
                ltemp = seq_map_check(x2o_ox(eoi))
                if (.not. ltemp) write(*,*) "MMMMMM mapper_Co2x: ", ltemp
            end if
#endif
...
#ifdef USE_C_COUPLER
            if (iamin_CPLID) then
                call mct_CCPL_register_fields(x2oacc_ox(eoi), cplocn_decomp_id, cplocn_grid_id, 0, "register x2oacc_ox")
                write(cTemp, '(i4)') eoi
                call mct_CCPL_register_export_interface(mapper_Cx2o(eoi), x2oacc_ox(eoi), cplocn_Cx2o_timer_id(eoi), comp_id=cpl_comp_id, mapper_name="mapper_Cx2o", avect_name="x2o_ox"//trim(adjustl(cTemp)), annotation="register export interface in cpl")
            end if
#endif
            x2oacc_ox_cnt = 0
         endif
...
#ifdef USE_C_COUPLER
            call use_ccpl_register_ice_grid(eii)
#endif
            call mct_avect_zero(x2i_ii(eii))
            call seq_map_map(mapper_Ci2x(eii), x2i_ii(eii), x2i_ix(eii), msgtag=CPLICEID(eii)*100+eii*10+3, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Ci2x(eii), x2i_ii(eii), x2i_ix(eii), msgtag=CPLICEID(eii)*100+eii*10+3)
            if (iamin_CPLID) then
                ltemp = seq_map_check(x2i_ix(eii))
                if (.not. ltemp) write(*,*) "MMMMMM x2i init mapper_Ci2x: ", ltemp, " ", iam_GLOID
            end if
#endif
         endif
      enddo
...
#ifdef USE_CPPL
            if (iamin_GLCID(egi)) then
                glc_grid_id(egi) = mct_CCPL_register_H2D_grid_via_local_data(glc_comp_id(egi), "glc_grid_at_glc_comp", "LON_LAT", "degrees",&
                    "cyclic", gsmap_gg(egi), dom_gg(egi), decomp_name="decomp_glc_grid_at_glc_comp", decomp_id=glc_decomp_id(egi), annotation="register glc grid in glc")
                call mct_CCPL_register_fields(x2g_gg(egi), glc_decomp_id(egi), glc_grid_id(egi), 0, "register x2g_gg")
                call mct_CCPL_register_fields(g2x_gg(egi), glc_decomp_id(egi), glc_grid_id(egi), 1, "register g2x_gg")
            end if
            if (iamin_CPLID) then
                if (egi == 1) then
                   cplglc_grid_id = mct_CCPL_register_H2D_grid_via_local_data(cpl_comp_id, "glc_grid_at_cpl_comp", "LON_LAT", & 
                       "degrees", "cyclic", gsmap_gx, dom_gx, decomp_name="decomp_glc_grid_at_cpl_comp", decomp_id=cplglc_decomp_id, annotation="register glc grid in cpl")
                end if
                call mct_CCPL_register_fields(x2g_gx(egi), cplglc_decomp_id, cplglc_grid_id, 10, "register x2g_gx")
                call mct_CCPL_register_fields(g2x_gx(egi), cplglc_decomp_id, cplglc_grid_id, 11, "register g2x_gx")
            end if
#endif
...
#ifdef USE_C_COUPLER
            call use_ccpl_register_wav_grid(ewi)
#endif
...
#ifdef USE_C_COUPLER
      call use_ccpl_register_cpl_fields()
#endif
   endif
...
#ifdef USE_C_COUPLER
      call use_ccpl_register_cpl_interface()
#endif

    endif ! iamin_CPLID

#ifdef USE_C_COUPLER
   call use_ccpl_end_coupling_configuration()
#endif
   ...
   if (atm_present) then
      do eai = 1,num_inst_atm
         if (iamin_CPLATMID(eai)) then
            ...
            call seq_map_map(mapper_Ca2x(eai), a2x_aa(eai), a2x_ax(eai), msgtag=CPLATMID(eai)*100+eai*10+7, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Ca2x(eai), a2x_aa(eai), a2x_ax(eai), msgtag=CPLATMID(eai)*100+eai*10+7)
            if (iamin_CPLID) then
                ltemp = seq_map_check(a2x_ax(eai))
                if (.not. ltemp) write(*,*) "MMMMMM init mapper_Ca2x: ", ltemp
            end if
#endif
         endif
   ...
   if (lnd_present) then
      do eli = 1,num_inst_lnd
         if (iamin_CPLLNDID(eli)) then
            ...
            call seq_map_map(mapper_Cl2x(eli), l2x_ll(eli), l2x_lx(eli), msgtag=CPLLNDID(eli)*100+eli*10+7, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Cl2x(eli), l2x_ll(eli), l2x_lx(eli), msgtag=CPLLNDID(eli)*100+eli*10+7)
            if (iamin_CPLID) then
                ltemp = seq_map_check(l2x_lx(eli))
                if (.not. ltemp) write(*,*) "MMMMMM init mapper_Cl2x: ", ltemp
            end if
#endif
         endif
   ...
   if (rof_present) then
      do eri = 1,num_inst_rof
         if (iamin_CPLROFID(eri)) then
            ...
            call seq_map_map(mapper_Cr2x(eri), r2x_rr(eri), r2x_rx(eri), msgtag=CPLROFID(eri)*100+eri*10+7, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Cr2x(eri), r2x_rr(eri), r2x_rx(eri), msgtag=CPLROFID(eri)*100+eri*10+7)
            if (iamin_CPLID) then
                ltemp = seq_map_check(r2x_rx(eri))
                if (.not. ltemp) write(*,*) "MMMMMM init mapper_Cr2x: ", ltemp
            end if
#endif
         endif
   ...
   if (ocn_present) then
      do eoi = 1,num_inst_ocn
         if (iamin_CPLOCNID(eoi)) then
           ...
           call seq_map_map(mapper_Co2x(eoi), o2x_oo(eoi), o2x_ox(eoi), msgtag=CPLOCNID(eoi)*100+eoi*10+7, use_ccpl=use_trans)
#ifdef BINARY_CHECK
           call seq_map_map(mapper_Co2x(eoi), o2x_oo(eoi), o2x_ox(eoi), msgtag=CPLOCNID(eoi)*100+eoi*10+7)
           if (iamin_CPLID) then
               ltemp = seq_map_check(o2x_ox(eoi))
               if (.not. ltemp) write(*,*) "MMMMMM init mapper_Co2x: ", ltemp
           end if
#endif
        endif
      enddo
   ...
   if (ice_present) then
      do eii = 1,num_inst_ice
         if (iamin_CPLICEID(eii)) then
            ...
            call seq_map_map(mapper_Ci2x(eii), i2x_ii(eii), i2x_ix(eii), msgtag=CPLICEID(eii)*100+eii*10+7, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Ci2x(eii), i2x_ii(eii), i2x_ix(eii), msgtag=CPLICEID(eii)*100+eii*10+7)
            if (iamin_CPLID) then
                ltemp = seq_map_check(i2x_ix(eii))
                if (.not. ltemp) write(*,*) "MMMMMM i2x init mapper_Ci2x: ", ltemp, " ", iam_GLOID
            end if
#endif
         endif
      enddo
...
#ifdef CCPL_FLUX
!            call use_ccpl_do_flux_atmocn_init( cdata_ox, a2x_ox(eai), o2x_ox(eoi), xao_ox(exi), &
!                   cpl_comp_id, cplocn_grid_id, cplocn_decomp_id, "use_ccpl_do_flux_atmocn_init ocn_grid")
#endif
         elseif (trim(aoflux_grid) == 'atm') then
            call seq_flux_init_mct(cdata_ax,fractions_ax(ens1))
#ifdef CCPL_FLUX
!            call use_ccpl_do_flux_atmocn_init( cdata_ax, a2x_ax(eai), o2x_ax(eoi), xao_ax(exi), &
!                   cpl_comp_id, cplatm_grid_id, cplatm_decomp_id, "use_ccpl_do_flux_atmocn_init atm_grid")
#endif
...
!tcx is this correct? relation between xao and frc for ifrad and ofrad
   ...
   if (atm_prognostic) then
         ...
         if (lnd_present) then
            ...
            do eli = 1,num_inst_lnd
               ...
               call seq_map_map(mapper_Sl2a,l2x_lx(eli),l2x_ax(eli), &
                                ...
                                avwts_s=fractions_lx(efi),avwtsfld_s='lfrin', use_ccpl=use_trans)
               ...
               call seq_map_map(mapper_Fl2a,l2x_lx(eli),l2x_ax(eli), &
                                fldlist=seq_flds_l2x_fluxes, norm=.true., &
                                avwts_s=fractions_lx(efi),avwtsfld_s='lfrin')
               ltemp = seq_map_check(l2x_ax(eli), fldlist=seq_flds_l2x_states)
               if (.not. ltemp) write(*,*) "MMMMMM mapper_Sl2a: ", ltemp
               ltemp = seq_map_check(l2x_ax(eli), fldlist=seq_flds_l2x_fluxes)
               if (.not. ltemp) write(*,*) "MMMMMM mapper_Fl2a: ", ltemp
#endif
            enddo
         endif
         if (ocn_present) then
            ...
            do eoi = 1,num_inst_ocn
               ...
               call seq_map_map(mapper_So2a,o2x_ox(eoi),o2x_ax(eoi),fldlist=seq_flds_o2x_states,norm=.true., &
                                avwts_s=fractions_ox(efi),avwtsfld_s='ofrac', use_ccpl=use_ccpl)
               call seq_map_map(mapper_Fo2a,o2x_ox(eoi),o2x_ax(eoi),fldlist=seq_flds_o2x_fluxes,norm=.true., use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
               call seq_map_map(mapper_So2a,o2x_ox(eoi),o2x_ax(eoi),fldlist=seq_flds_o2x_states,norm=.true., &
               ...
               ltemp = seq_map_check(o2x_ax(eoi), fldlist=seq_flds_o2x_states)
               if (.not. ltemp) write(*,*) "MMMMMM init mapper_So2a: ", ltemp
               ltemp = seq_map_check(o2x_ax(eoi), fldlist=seq_flds_o2x_fluxes)
               if (.not. ltemp) write(*,*) "MMMMMM init mapper_Fo2a: ", ltemp
#endif
            enddo
            do exi = 1,num_inst_xao
               efi = mod((exi-1),num_inst_frc) + 1
               call seq_map_map(mapper_So2a,xao_ox(exi),xao_ax(exi),fldlist=seq_flds_xao_albedo, norm=.true., &
                                avwts_s=fractions_ox(efi),avwtsfld_s='ofrac', use_ccpl=use_ccpl, stage="albedo")
#ifdef BINARY_CHECK
               call seq_map_map(mapper_So2a,xao_ox(exi),xao_ax(exi),fldlist=seq_flds_xao_albedo, norm=.true., &
                                avwts_s=fractions_ox(efi),avwtsfld_s='ofrac')
               ltemp = seq_map_check(xao_ax(exi), fldlist=seq_flds_xao_albedo) 
               if (.not. ltemp) write(*,*) "MMMMMM xao albedo mapper_So2a: ", ltemp
#endif
               if (trim(aoflux_grid) == 'ocn') then
                  ...
                  call seq_map_map(mapper_So2a,xao_ox(exi),xao_ax(exi),fldlist=seq_flds_xao_states,norm=.true., &
                                   avwts_s=fractions_ox(efi),avwtsfld_s='ofrac', use_ccpl=use_ccpl, stage="states")
#ifdef BINARY_CHECK
                  call seq_map_map(mapper_So2a,xao_ox(exi),xao_ax(exi),fldlist=seq_flds_xao_states,norm=.true., &
                                   avwts_s=fractions_ox(efi),avwtsfld_s='ofrac')
                  ltemp = seq_map_check(xao_ax(exi),fldlist=seq_flds_xao_states)
                  if (.not. ltemp) write(*,*) "MMMMMM xao states mapper_So2a: ", ltemp
#endif
                  ...
                  call seq_map_map(mapper_Fo2a,xao_ox(exi),xao_ax(exi),fldlist=seq_flds_xao_fluxes,norm=.true., &
                                   avwts_s=fractions_ox(efi),avwtsfld_s='ofrac')
                  ltemp = seq_map_check(xao_ax(exi),fldlist=seq_flds_xao_fluxes)
                  if (.not. ltemp) write(*,*) "MMMMMM xao fluxes mapper_So2a: ", ltemp
#endif
               endif
               if (trim(aoflux_grid) == 'atm') then
                  ...
                  call seq_map_map(mapper_Fa2o, xao_ax(exi), xao_ox(exi), norm=.true., use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
                  call seq_map_map(mapper_Fa2o, xao_ax(exi), xao_ox(exi), norm=.true.)
                  ltemp = seq_map_check(xao_ox(exi))
                  if (.not. ltemp) write(*, *) "MMMMMM mapper_Fa2o: ", ltemp
#endif
               endif
            enddo
         endif
         if (ice_present) then
            ...
            do eii = 1,num_inst_ice
               ...
               call seq_map_map(mapper_Si2a, i2x_ix(eii), i2x_ax(eii), fldlist=seq_flds_i2x_states, &
                    avwts_s=fractions_ix(eii), avwtsfld_s='ifrac', use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
               call seq_map_map(mapper_Si2a, i2x_ix(eii), i2x_ax(eii), fldlist=seq_flds_i2x_states, &
                    avwts_s=fractions_ix(eii), avwtsfld_s='ifrac')
               ltemp = seq_map_check(i2x_ax(eii), fldlist=seq_flds_i2x_states)
               if (.not. ltemp) write(*,*) "MMMMMM mapper_Si2a: ", ltemp
#endif
               ...
               call seq_map_map(mapper_Fi2a, i2x_ix(eii), i2x_ax(eii), fldlist=seq_flds_i2x_fluxes, &
                    avwts_s=fractions_ix(eii), avwtsfld_s='ifrac')
               ltemp = seq_map_check(i2x_ax(eii), fldlist=seq_flds_i2x_fluxes)
               if (.not. ltemp) write(*,*) "MMMMMM mapper_Fi2a: ", ltemp
#endif
            enddo
         endif
      ...
      do eai = 1,num_inst_atm
         if (iamin_CPLATMID(eai)) then
            call seq_map_map(mapper_Cx2a(eai), x2a_ax(eai), x2a_aa(eai), msgtag=CPLATMID(eai)*100+eai*10+2, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Cx2a(eai), x2a_ax(eai), x2a_aa(eai), msgtag=CPLATMID(eai)*100+eai*10+2)
            if (iamin_ATMID(eai)) then
                ltemp = seq_map_check(x2a_aa(eai))
                if (.not. ltemp) write(*,*) "MMMMMM init mapper_Cx2a: ", ltemp
            end if
#endif
         endif
      enddo
   ...
   if (atm_present) then
      ...
      do eai = 1,num_inst_atm
         if (iamin_CPLATMID(eai)) then
            call seq_map_map(mapper_Ca2x(eai), a2x_aa(eai), a2x_ax(eai), msgtag=CPLATMID(eai)*100+eai*10+4, use_ccpl=use_trans)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Ca2x(eai), a2x_aa(eai), a2x_ax(eai), msgtag=CPLATMID(eai)*100+eai*10+4)
            if (iamin_CPLID) then
                ltemp = seq_map_check(a2x_ax(eai))
                if (.not. ltemp) write(*,*) "MMMMMM mapper_Ca2x: ", ltemp
            end if
#endif
         endif
      enddo
 ...
 107  format( A, 2i8, A, f12.4, A, f12.4 )
   ...
   do while ( .not. stop_alarm)
         ...
         if (loglevel > 1) then
            write(logunit,102) ' Alarm_state: model date = ',ymd,tod, &
               ' aliogrw run alarms = ',  atmrun_alarm, lndrun_alarm, &
                         ...
                         rofrun_alarm, wavrun_alarm, ocnnext_alarm
            write(logunit,102) ' Alarm_state: model date = ',ymd,tod, &
      ...
      if (iamin_CPLID .and. (ice_present.or.ocn_present) .and. atm_present) then
         ...
         do eai = 1,num_inst_atm
            call seq_map_map(mapper_Sa2o, a2x_ax(eai), a2x_ox(eai), fldlist=seq_flds_a2x_states, norm=.true., use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Sa2o, a2x_ax(eai), a2x_ox(eai), fldlist=seq_flds_a2x_states, norm=.true.)
            ltemp = seq_map_check(a2x_ox(eai), fldlist=seq_flds_a2x_states_no_uv)
            if (.not. ltemp) write(*,*) "MMMMMM mapper_Sa2o: ", ltemp
#endif
            call seq_map_map(mapper_Fa2o, a2x_ax(eai), a2x_ox(eai), fldlist=seq_flds_a2x_fluxes, norm=.true., use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Fa2o, a2x_ax(eai), a2x_ox(eai), fldlist=seq_flds_a2x_fluxes, norm=.true.)
            ltemp = seq_map_check(a2x_ox(eai), fldlist=seq_flds_a2x_fluxes)
            if (.not. ltemp) write(*,*) "MMMMMM mapper_Fa2o: ", ltemp
#endif
            !--- tcx this Va2o call will not be necessary when npfix goes away
            call seq_map_map(mapper_Va2o, a2x_ax(eai), a2x_ox(eai), fldlist='Sa_u:Sa_v', norm=.true., use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
            call seq_map_map(mapper_Va2o, a2x_ax(eai), a2x_ox(eai), fldlist='Sa_u:Sa_v', norm=.true.)
            ltemp = seq_map_check(a2x_ox(eai), fldlist='Sa_u:Sa_v')
            if (.not. ltemp) write(*,*) "MMMMMM mapper_Va2o: ", ltemp
#endif
...
!              call mct_aVect_average(x2oacc_ox(eoi))
               ...
               if (r2xacc_rx_cnt > 0) then
                  ...
                  do eri = 1,num_inst_rof
                     call seq_map_map(mapper_Rr2o, r2xacc_rx(eri), r2x_ox(eri), &
                                      fldlist='Forr_roff:Forr_ioff', norm=.false., use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
                     call seq_map_map(mapper_Rr2o, r2xacc_rx(eri), r2x_ox(eri), &
                                      fldlist='Forr_roff:Forr_ioff', norm=.false.)
                     ltemp = seq_map_check(r2x_ox(eri), fldlist='Forr_roff:Forr_ioff')
                     if (.not. ltemp) write(*,*) "MMMMMM mapper_Rr2o: ", ltemp
#endif
                     if (flood_present) then
                        call seq_map_map(mapper_Fr2o, r2xacc_rx(eri), r2x_ox(eri), &
                                         fldlist='Flrr_flood', norm=.true., use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
                        call seq_map_map(mapper_Fr2o, r2xacc_rx(eri), r2x_ox(eri), &
                                         fldlist='Flrr_flood', norm=.true.)
                        ltemp = seq_map_check(r2x_ox(eri), fldlist='Flrr_flood')
                        if (.not. ltemp) write(*,*) "MMMMMM mapper_Fr2o: ", ltemp
#endif
         ...
         if (iamin_CPLALLOCNID .and. ocn_prognostic) then
            ...
            do eoi = 1,num_inst_ocn
               if (iamin_CPLOCNID(eoi)) then
                  ...
                  call seq_map_map(mapper_Cx2o(eoi), x2oacc_ox(eoi), x2o_oo(eoi), msgtag=CPLOCNID(eoi)*100+eoi*10+2, use_ccpl=use_trans)
#ifdef BINARY_CHECK
                  call seq_map_map(mapper_Cx2o(eoi), x2oacc_ox(eoi), x2o_oo(eoi), msgtag=CPLOCNID(eoi)*100+eoi*10+2)
                  if (iamin_OCNID(eoi)) then
                      ltemp = seq_map_check(x2o_oo(eoi))
                      if (.not. ltemp) write(*,*) "MMMMMM mapper_Cx2o: ", ltemp
                  end if
#endif
      ...
      if (lnd_present .and. lndrun_alarm) then
         ...
         if (iamin_CPLID) then
            ...
            if (lnd_prognostic) then
               ...
               do eai = 1,num_inst_atm
                  call seq_map_map(mapper_Fa2l, a2x_ax(eai), a2x_lx(eai), norm=.true., use_ccpl=use_trans)
#ifdef BINARY_CHECK
                  call seq_map_map(mapper_Fa2l, a2x_ax(eai), a2x_lx(eai), norm=.true.)
                  ltemp = seq_map_check(a2x_lx(eai))
                  if (.not. ltemp) write(*,*) "MMMMMM mapper_Fa2l: ", ltemp
#endif
               enddo
               ...
               if (flood_present) then
                  ...
                  do eri = 1,num_inst_rof
                     ...
                     call seq_map_map(mapper_Fr2l, r2x_rx(eri), r2x_lx(eri), &
                          fldlist=seq_flds_r2x_fluxes, norm=.true., use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
                     call seq_map_map(mapper_Fr2l, r2x_rx(eri), r2x_lx(eri), &
                          fldlist=seq_flds_r2x_fluxes, norm=.true.)
                     ltemp = seq_map_check(r2x_lx(eri), fldlist=seq_flds_r2x_fluxes)
                     if (.not. ltemp) write(*,*) "MMMMMM mapper_Fr2l: ", ltemp
#endif
                  end do
               ...
               if (rof_present) then
                  ...
                  do eri = 1,num_inst_rof
                     ...
                     call seq_map_map(mapper_Sr2l,r2x_rx(eri),r2x_lx(eri), &
                                fldlist=seq_flds_r2x_states, norm=.true., use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
                     call seq_map_map(mapper_Sr2l,r2x_rx(eri),r2x_lx(eri), &
                                fldlist=seq_flds_r2x_states, norm=.true.)
                     ltemp = seq_map_check(r2x_lx(eri), fldlist=seq_flds_r2x_states)
                     if (.not. ltemp) write(*,*) "MMMMMM mapper_Sr2l: ", ltemp
#endif
                  end do
                  call t_drvstopf  ('driver_lndprep_rof2lnd')
               end if
            ...
            do eli = 1,num_inst_lnd
               if (iamin_CPLLNDID(eli)) then
                  if (lnd_prognostic) then
                     ...
                     call seq_map_map(mapper_Cx2l(eli), x2l_lx(eli), x2l_ll(eli), msgtag=CPLLNDID(eli)*100+eli*10+2, use_ccpl=use_trans)
#ifdef BINARY_CHECK
                     call seq_map_map(mapper_Cx2l(eli), x2l_lx(eli), x2l_ll(eli), msgtag=CPLLNDID(eli)*100+eli*10+2)
                     if (iamin_LNDID(eli)) then
                         ltemp = seq_map_check(x2l_ll(eli))
                         if (.not. ltemp) write(*,*) "MMMMMM mapper_Cx2l: ", ltemp
                     end if
#endif
      ...
      if (ice_present .and. icerun_alarm) then
         ...
         if (iamin_CPLID .and. ice_prognostic) then
            ...
            do eoi = 1,num_inst_ocn
               call seq_map_map(mapper_SFo2i, o2x_ox(eoi), o2x_ix(eoi), norm=.true., use_ccpl=use_trans)
#ifdef BINARY_CHECK
               call seq_map_map(mapper_SFo2i, o2x_ox(eoi), o2x_ix(eoi), norm=.true.)
               ltemp = seq_map_check(o2x_ix(eoi))
               if (.not. ltemp) write(*,*) "MMMMMM mapper_SFo2i: ", ltemp
#endif
            enddo
            ...
            do eai = 1,num_inst_atm
               call seq_map_map(mapper_SFo2i, a2x_ox(eai), a2x_ix(eai), norm=.true., use_ccpl=use_trans)
#ifdef BINARY_CHECK
               call seq_map_map(mapper_SFo2i, a2x_ox(eai), a2x_ix(eai), norm=.true.)
               ltemp = seq_map_check(a2x_ix(eai))
               if (.not. ltemp) write(*,*) "MMMMMM mapper_SFo2i: ", ltemp
#endif
         ...
         if (iamin_CPLALLICEID .and. ice_prognostic) then
            ...
            do eii = 1,num_inst_ice
               if (iamin_CPLICEID(eii)) then
                  ...
                  call seq_map_map(mapper_Cx2i(eii), x2i_ix(eii), x2i_ii(eii), msgtag=CPLICEID(eii)*100+eii*10+2, use_ccpl=use_trans)
#ifdef BINARY_CHECK
                  call seq_map_map(mapper_Cx2i(eii), x2i_ix(eii), x2i_ii(eii), msgtag=CPLICEID(eii)*100+eii*10+2)
                  if (iamin_ICEID(eii)) then
                      ltemp = seq_map_check(x2i_ii(eii))
                      if (.not. ltemp) write(*,*) "MMMMMM mapper_Cx2i: ", ltemp
                  end if
#endif
         ...
         if (iamin_CPLID .and. rof_prognostic) then
            ...
            do eri = 1,num_inst_rof
               ...
               call seq_map_map(mapper_Fl2r, x2racc_lx(eli), x2r_rx_tmp, &
                  ...
                  avwts_s=fractions_lx(efi), avwtsfld_s='lfrin', use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
               call seq_map_map(mapper_Fl2r, x2racc_lx(eli), x2r_rx_tmp, &
                  fldlist=seq_flds_x2r_fluxes, norm=.true., &
                  avwts_s=fractions_lx(efi), avwtsfld_s='lfrin')
               ltemp = seq_map_check(x2r_rx_tmp, fldlist=seq_flds_x2r_fluxes)
               if (.not. ltemp) write(*,*) "MMMMMM mapper_Fl2r: ", ltemp
#endif
         ...
         if (iamin_CPLALLROFID .and. rof_prognostic) then
            ...
            do eri = 1,num_inst_rof
               if (iamin_CPLROFID(eri)) then
                  ...
                  call seq_map_map(mapper_Cx2r(eri), x2r_rx(eri), x2r_rr(eri), msgtag=CPLROFID(eri)*100+eri*10+2, use_ccpl=use_trans)
#ifdef BINARY_CHECK
                  call seq_map_map(mapper_Cx2r(eri), x2r_rx(eri), x2r_rr(eri), msgtag=CPLROFID(eri)*100+eri*10+2)
                  if (iamin_ROFID(eri)) then
                      ltemp = seq_map_check(x2r_rr(eri))
                      if (.not. ltemp) write(*,*) "MMMMMM mapper_Cx2r: ", ltemp
                  end if
#endif
      ...
      if (ocn_present .and. ocnnext_alarm) then
         ...
         do eoi = 1,num_inst_ocn
            if (iamin_CPLOCNID(eoi)) then
               ...
               call seq_map_map(mapper_Co2x(eoi), o2x_oo(eoi), o2x_ox(eoi), msgtag=CPLOCNID(eoi)*100+eoi*10+4, use_ccpl=use_trans)
#ifdef BINARY_CHECK
      ...
      if (ocn_present .and. iamin_CPLID) then
         ...
         if (ocn_prognostic) then
            ...
            if (ice_present) then
               ...
               do eii = 1,num_inst_ice
                  call seq_map_map(mapper_SFi2o, i2x_ix(eii), i2x_ox(eii), norm=.true., use_ccpl=use_trans)
#ifdef BINARY_CHECK
                  call seq_map_map(mapper_SFi2o, i2x_ix(eii), i2x_ox(eii), norm=.true.)
                  ltemp = seq_map_check(i2x_ox(eii))
                  if (.not. ltemp) write(*,*) "MMMMMM mapper_SFi2o: ", ltemp
#endif
               enddo
...
!     !        call mct_aVect_accumulate(x2o_ox(eoi), x2oacc_ox(eoi))
         ...
         do exi = 1,num_inst_xao
            ...
            if (trim(aoflux_grid) == 'ocn') then
               ...
               call use_ccpl_do_flux_atmocn_init( cdata_ox, a2x_ox(eai), o2x_ox(eoi), xao_ox(exi), &
                   cpl_comp_id, cplocn_grid_id, cplocn_decomp_id, "use_ccpl_do_flux_atmocn_init ocn_grid")
               call use_ccpl_do_flux_atmocn_run( a2x_ox(eai), o2x_ox(eoi), xao_ox(exi))
 #ifdef BINARY_CHECK_FLUX
               call seq_flux_atmocn_mct( cdata_ox, a2x_ox(eai), o2x_ox(eoi), xao_ox(exi))
               if (iamin_CPLID) then
                   ltemp = seq_map_check(xao_ox(exi),'So_tref:So_qref:So_ustar:So_re:So_ssq:So_u10:So_duu10n:&
                                              Faox_taux:Faox_tauy:Faox_lat:Faox_sen:Faox_evap:Faox_lwup')
                   if (.not. ltemp) write(*,*) "MMMMMM use_ccpl_do_flux_atmocn ocn_grid: ", ltemp, " ", iam_GLOID
                   !write(*,*) "MMMMMM use_ccpl_do_flux_atmocn ocn_grid: ", ltemp
               end if
 #endif
#else
               call seq_flux_atmocn_mct( cdata_ox, a2x_ox(eai), o2x_ox(eoi), xao_ox(exi))
         ...
         do eli = 1,num_inst_lnd
            ...
            if (iamin_CPLLNDID(eli)) then
               if (lnd_present) then
                  ...
                  call seq_map_map(mapper_Cl2x(eli), l2x_ll(eli), l2x_lx(eli), msgtag=CPLLNDID(eli)*100+eli*10+6, use_ccpl=use_trans)
#ifdef BINARY_CHECK
                  call seq_map_map(mapper_Cl2x(eli), l2x_ll(eli), l2x_lx(eli), msgtag=CPLLNDID(eli)*100+eli*10+6)
                  if (iamin_CPLID) then
                      ltemp = seq_map_check(l2x_lx(eli))
                      if (.not. ltemp) write(*,*) "MMMMMM mapper_Cl2x: ", ltemp
                  end if
#endif
      ...
      if (iamin_CPLALLROFID) then
         ...
         do eri = 1,num_inst_rof
            if (iamin_CPLROFID(eri)) then
               ...
               call seq_map_map(mapper_Cr2x(eri), r2x_rr(eri), r2x_rx(eri), msgtag=CPLROFID(eri)*100+eri*10+4, use_ccpl=use_trans)
#ifdef BINARY_CHECK
               call seq_map_map(mapper_Cr2x(eri), r2x_rr(eri), r2x_rx(eri), msgtag=CPLROFID(eri)*100+eri*10+4)
               if (iamin_CPLID) then
                   ltemp = seq_map_check(r2x_rx(eri))
                   if (.not. ltemp) write(*,*) "MMMMMM mapper_Cr2x: ", ltemp
               end if
#endif
      ...
      if (iamin_CPLALLICEID) then
         ...
         do eii = 1,num_inst_ice
            if (iamin_CPLICEID(eii)) then
               ...
               call seq_map_map(mapper_Ci2x(eii), i2x_ii(eii), i2x_ix(eii), msgtag=CPLICEID(eii)*100+eii*10+4, use_ccpl=use_trans)
#ifdef BINARY_CHECK
               call seq_map_map(mapper_Ci2x(eii), i2x_ii(eii), i2x_ix(eii), msgtag=CPLICEID(eii)*100+eii*10+4)
               if (iamin_CPLID) then
                   ltemp = seq_map_check(i2x_ix(eii))
                   if (.not. ltemp) write(*,*) "MMMMMM mapper_Ci2x: ", ltemp, " ", iam_GLOID
               end if
#endif
         ...
         if (trim(aoflux_grid) == 'atm') then
            ...
            do eoi = 1,num_inst_ocn
               ...
               call seq_map_map(mapper_So2a,o2x_ox(eoi),o2x_ax(eoi),fldlist=seq_flds_o2x_states,norm=.true., &
                                avwts_s=fractions_ox(efi),avwtsfld_s='ofrac')
               ltemp = seq_map_check(o2x_ax(eoi),fldlist=seq_flds_o2x_states)
               if (.not. ltemp) write(*,*) "MMMMMM mapper_So2a: ", ltemp, " ", iam_GLOID
#endif
               !ltemp = seq_map_check(o2x_ox(eoi),fldlist=seq_flds_o2x_states)
               !write(*,*) "QQQQQQ mapper_So2a: ", ltemp, " ", iam_GLOID
               ...
               call seq_map_map(mapper_Fo2a,o2x_ox(eoi),o2x_ax(eoi),fldlist=seq_flds_o2x_fluxes,norm=.true.)
               ltemp = seq_map_check(o2x_ax(eoi),fldlist=seq_flds_o2x_fluxes)
               if (.not. ltemp) write(*,*) "MMMMMM mapper_Fo2a: ", ltemp, " ", iam_GLOID
#endif
            ...
            do exi = 1,num_inst_xao 
               ...
               call use_ccpl_do_flux_atmocn_init( cdata_ax, a2x_ax(eai), o2x_ax(eoi), xao_ax(exi), &
                   cpl_comp_id, cplatm_grid_id, cplatm_decomp_id, "use_ccpl_do_flux_atmocn_init atm_grid")
               call use_ccpl_do_flux_atmocn_run(a2x_ax(eai), o2x_ax(eoi), xao_ax(exi))
 #ifdef BINARY_CHECK_FLUX
               call seq_flux_atmocn_mct( cdata_ax, a2x_ax(eai), o2x_ax(eoi), xao_ax(exi))
               if (iamin_CPLID) then
                   ltemp = seq_map_check(xao_ox(exi),'So_tref:So_qref:So_ustar:So_re:So_ssq:So_u10:So_duu10n:&
                                              Faox_taux:Faox_tauy:Faox_lat:Faox_sen:Faox_evap:Faox_lwup')
                   if (.not. ltemp) write(*,*) "MMMMMM use_ccpl_do_flux_atmocn atm_grid: ", ltemp, " ", iam_GLOID
                   write(*,*) "MMMMMM use_ccpl_do_flux_atmocn atm_grid: ", ltemp
               end if
 #endif
#else
               call seq_flux_atmocn_mct( cdata_ax, a2x_ax(eai), o2x_ax(eoi), xao_ax(exi))
         ...
         if (iamin_CPLID .and. atm_prognostic) then
               ...
               if (trim(aoflux_grid) == 'ocn') then
                  ...
                  do exi = 1,num_inst_xao
                     ...
                     call seq_map_map(mapper_Fo2a,xao_ox(exi),xao_ax(exi),fldlist=seq_flds_xao_fluxes,norm=.true., &
                                      avwts_s=fractions_ox(efi),avwtsfld_s='ofrac')
                     ltemp = seq_map_check(xao_ax(exi),fldlist=seq_flds_xao_fluxes)
                     if (.not. ltemp) write(*,*) "MMMMMM xao fluxes mapper_Fo2a: ", ltemp
#endif
                  enddo
               ...
               if (trim(aoflux_grid) /= 'atm') then
                  do eoi = 1,num_inst_ocn
                     ...
                     call seq_map_map(mapper_So2a,o2x_ox(eoi),o2x_ax(eoi),fldlist=seq_flds_o2x_states,norm=.true., &
                                      avwts_s=fractions_ox(efi),avwtsfld_s='ofrac')
                     ltemp = seq_map_check(o2x_ax(eoi), fldlist=seq_flds_o2x_states)
                     if (.not. ltemp) write(*,*) "MMMMMM xao mapper_So2a: ", ltemp
#endif
                     ...
                     call seq_map_map(mapper_Fo2a,o2x_ox(eoi),o2x_ax(eoi),fldlist=seq_flds_o2x_fluxes,norm=.true.)
                     ltemp = seq_map_check(o2x_ax(eoi), fldlist=seq_flds_o2x_fluxes)
                     if (.not. ltemp) write(*,*) "MMMMMM xao mapper_Fo2a: ", ltemp
#endif
            ...
            if (ice_present) then
               ...
               do eii = 1,num_inst_ice
                  efi = mod((eii-1),num_inst_frc) + 1
                  call seq_map_map(mapper_Si2a, i2x_ix(eii), i2x_ax(eii), fldlist=seq_flds_i2x_states, &
                       avwts_s=fractions_ix(efi), avwtsfld_s='ifrac', use_ccpl=use_ccpl)
#ifdef BINARY_CHECK
                  call seq_map_map(mapper_Si2a, i2x_ix(eii), i2x_ax(eii), fldlist=seq_flds_i2x_states, &
...
!            call seq_hist_spewav('x2a_ax(eai)',gsmap_ax,x2a_ax(eai),atm_nx,atm_ny,1)
         ...
         if (atm_prognostic) then
            ...
            do eai = 1,num_inst_atm
               if (iamin_CPLATMID(eai)) then
                  ...
                  if (iamin_ATMID(eai)) then
                      ltemp = seq_map_check(x2a_aa(eai))
                      if (.not. ltemp) write(*,*) "MMMMMM mapper_Cx2a: ", ltemp, " ", iam_GLOID
                  end if
#endif
      ...
      if (ocn_present .and. ocnnext_alarm) then
         ...
         do eoi = 1,num_inst_ocn
            if (iamin_CPLOCNID(eoi)) then
               ...
               call seq_map_map(mapper_Co2x(eoi), o2x_oo(eoi), o2x_ox(eoi), msgtag=CPLOCNID(eoi)*100+eoi*10+6, use_ccpl=use_trans)
#ifdef BINARY_CHECK
      ...
      if (restart_alarm) then
          call use_ccpl_do_restart_write()
      endif
#endif
...
subroutine use_ccpl_get_timer()
   call seq_timemgr_EClockGetData( EClock_d, dtime=dtime)
   call seq_timemgr_EClockGetData( EClock_l, dtime=dtimel)
   call seq_timemgr_EClockGetData( EClock_i, dtime=dtimei)
   call seq_timemgr_EClockGetData( EClock_g, dtime=dtimeg)
   call seq_timemgr_EClockGetData( EClock_w, dtime=dtimew)
end subroutine use_ccpl_get_timer

subroutine use_ccpl_register_component()
   root_comp_id = CCPL_register_component(-1, "cesm", "active_coupled_system", mpicom_GLOID, annotation="root component cesm start registration")
   if (trim(aoflux_grid) == 'atm' .and. iamin_CPLID .and. ocn_present) then
       xao_comp_id = CCPL_register_component(root_comp_id, "xao", "cpl", mpicom_CPLID, annotation="component xao start registration")
       call CCPL_set_normal_time_step(xao_comp_id, dtime)
       xao_timer_id = CCPL_define_single_timer(xao_comp_id, "steps", 1, 1, annotation="define a single timer for xao")
   end if
   if (iamin_CPLID) then
       cpl_comp_id = CCPL_register_component(root_comp_id, "cpl", "cpl", mpicom_CPLID, annotation="component cpl7 start registration")
       call CCPL_set_normal_time_step(cpl_comp_id, dtime)
       cpl_timer_id = CCPL_define_single_timer(cpl_comp_id, "steps", 1, 1, annotation="define a single timer for cpl7")
       do eai = 1, num_inst_atm
           cplatm_timer_id(eai) = CCPL_define_single_timer(cpl_comp_id, "steps", dtimea/dtime, 1, annotation="define a single timer for cpl7")
           end do
       do eli = 1, num_inst_lnd
           cpllnd_timer_id(eli) = CCPL_define_single_timer(cpl_comp_id, "steps", dtimel/dtime, 1, annotation="define a single timer for cpl7")
       end do
       do eoi = 1, num_inst_ocn
           cplocn_Cx2o_timer_id(eoi) = CCPL_define_single_timer(cpl_comp_id, "steps", dtimeo/dtime, dtimeo/dtime+1, annotation="define a single timer for cpl7")
           cplocn_Co2x_timer_id(eoi) = CCPL_define_single_timer(cpl_comp_id, "steps", dtimeo/dtime, dtimeo/dtime*2, annotation="define a single timer for cpl7")
       end do
       do eii = 1, num_inst_ice
           cplice_timer_id(eii) = CCPL_define_single_timer(cpl_comp_id, "steps", dtimei/dtime, 1, annotation="define a single timer for cpl7")
       end do
       do eri = 1, num_inst_rof
           cplrof_timer_id(eri) = CCPL_define_single_timer(cpl_comp_id, "steps", dtimer/dtime, 1, annotation="define a single timer for cpl7")
           end do
       do ewi = 1, num_inst_wav
           cplwav_timer_id(ewi) = CCPL_define_single_timer(cpl_comp_id, "steps", dtimew/dtime, 1, annotation="define a single timer for cpl7")
       end do
       do egi = 1, num_inst_glc
           cplglc_timer_id(egi) = CCPL_define_single_timer(cpl_comp_id, "steps", dtimeg/dtime, 1, annotation="define a single timer for cpl7")
       end do
   end if
   do eai = 1, num_inst_atm
       if (iamin_ATMID(eai) .and. atm_present) then
          atm_comp_id(eai) = CCPL_register_component(root_comp_id, trim(atm_name(eai)), "atm", mpicom_ATMID(eai), annotation="component "//trim(atm_name(eai))//" start registration")
          call CCPL_set_normal_time_step(atm_comp_id(eai), dtime)
          atm_timer_id(eai) = CCPL_define_single_timer(atm_comp_id(eai), "steps", dtimea/dtime, 1, annotation="define a single timer for "//trim(atm_name(eai)))
       end if
   end do
   do eli = 1, num_inst_lnd
       if (iamin_LNDID(eli) .and. lnd_present) then
          lnd_comp_id(eli) = CCPL_register_component(root_comp_id, trim(lnd_name(eli)), "lnd", mpicom_LNDID(eli), annotation="component "//trim(lnd_name(eli))//" start registration")
          call CCPL_set_normal_time_step(lnd_comp_id(eli), dtime)
          lnd_timer_id(eli) = CCPL_define_single_timer(lnd_comp_id(eli), "steps", dtimel/dtime, 1, annotation="define a single timer for "//trim(lnd_name(eli)))
       end if
   end do
   do eii = 1, num_inst_ice
       if (iamin_ICEID(eii) .and. ice_present) then
          ice_comp_id(eii) = CCPL_register_component(root_comp_id, trim(ice_name(eii)), "sea_ice", mpicom_ICEID(eii), annotation="component "//trim(ice_name(eii))//" start registration")
          call CCPL_set_normal_time_step(ice_comp_id(eii), dtime)
          ice_timer_id(eii) = CCPL_define_single_timer(ice_comp_id(eii), "steps", dtimei/dtime, 1, annotation="define a single timer for "//trim(ice_name(eii)))
       end if
   end do
   do eoi = 1, num_inst_ocn
       if (iamin_OCNID(eoi) .and. ocn_present) then
          ocn_comp_id(eoi) = CCPL_register_component(root_comp_id, trim(ocn_name(eoi)), "ocn", mpicom_OCNID(eoi), annotation="component "//trim(ocn_name(eoi))//" start registration")
          call CCPL_set_normal_time_step(ocn_comp_id(eoi), dtime)
          ocn_Cx2o_timer_id(eoi) = CCPL_define_single_timer(ocn_comp_id(eoi), "steps", dtimeo/dtime, dtimeo/dtime+1, annotation="define a single timer for "//trim(ocn_name(eoi)))
          ocn_Co2x_timer_id(eoi) = CCPL_define_single_timer(ocn_comp_id(eoi), "steps", dtimeo/dtime, dtimeo/dtime*2, annotation="define a single timer for "//trim(ocn_name(eoi)))
       end if
   end do
   do egi = 1, num_inst_glc
       if (iamin_GLCID(egi) .and. glc_present) then
          glc_comp_id(egi) = CCPL_register_component(root_comp_id, trim(glc_name(egi)), "glc", mpicom_GLCID(egi), annotation="component "//trim(glc_name(egi))//" start registration")
          call CCPL_set_normal_time_step(glc_comp_id(egi), dtime)
          glc_timer_id(egi) = CCPL_define_single_timer(glc_comp_id(egi), "steps", dtimeg/dtime, 1, annotation="define a single timer for "//trim(glc_name(egi)))
       end if
   end do
   do eri = 1, num_inst_rof
       if (iamin_ROFID(eri) .and. rof_present) then
          rof_comp_id(eri) = CCPL_register_component(root_comp_id, trim(rof_name(eri)), "roff", mpicom_ROFID(eri), annotation="component "//trim(rof_name(eri))//" start registration")
          call CCPL_set_normal_time_step(rof_comp_id(eri), dtime)
          rof_timer_id(eri) = CCPL_define_single_timer(rof_comp_id(eri), "steps", dtimer/dtime, 1, annotation="define a single timer for "//trim(rof_name(eri)))
       end if
   end do
   do ewi = 1, num_inst_wav
       if (iamin_WAVID(ewi) .and. wav_present) then
          wav_comp_id(ewi) = CCPL_register_component(root_comp_id, trim(wav_name(ewi)), "wave", mpicom_WAVID(ewi), annotation="component "//trim(wav_name(ewi))//" start registration")
          call CCPL_set_normal_time_step(wav_comp_id(ewi), dtime)
          wav_timer_id(ewi) = CCPL_define_single_timer(wav_comp_id(ewi), "steps", dtimer/dtime, 1, annotation="define a single timer for "//trim(wav_name(ewi)))
       end if
   end do
end subroutine use_ccpl_register_component

subroutine use_ccpl_register_atm_grid(eai)
    implicit none
    integer, intent(in) :: eai
    if (iamin_ATMID(eai)) then
        atm_grid_id(eai) = mct_CCPL_register_H2D_grid_via_local_data(atm_comp_id(eai), "atm_grid_at_atm_comp", "LON_LAT", "degrees",&
                "cyclic", gsmap_aa(eai), dom_aa(eai), decomp_name="decomp_atm_grid_at_atm_comp", decomp_id=atm_decomp_id(eai), annotation="register atm grid in atm")
        call mct_CCPL_register_fields(a2x_aa(eai), atm_decomp_id(eai), atm_grid_id(eai), 0, "register a2x_aa")
        call mct_CCPL_register_fields(x2a_aa(eai), atm_decomp_id(eai), atm_grid_id(eai), 1, "register x2a_aa")
        write(cTemp, '(i4)') eai
        call mct_CCPL_register_import_interface(mapper_Cx2a(eai), x2a_aa(eai), atm_timer_id(eai), 0, comp_id=atm_comp_id(eai), mapper_name="mapper_Cx2a", avect_name="x2a_aa"//trim(adjustl(cTemp)), annotation="register import interface in atm")
        call mct_CCPL_register_export_interface(mapper_Ca2x(eai), a2x_aa(eai), atm_timer_id(eai), comp_id=atm_comp_id(eai), mapper_name="mapper_Ca2x", avect_name="a2x_aa"//trim(adjustl(cTemp)), annotation="register export interface in atm")
        call mct_CCPL_register_export_interface(mapper_Ca2x(eai), x2a_aa(eai), atm_timer_id(eai), comp_id=atm_comp_id(eai), mapper_name="mapper_Ca2x", avect_name="x2a_aa"//trim(adjustl(cTemp)), annotation="register export interface in atm")
    end if
    if (iamin_CPLID) then
        !edited by Zhang Cheng
        if (eai == 1) then
            cplatm_grid_id = mct_CCPL_register_H2D_grid_via_local_data(cpl_comp_id, "atm_grid_at_cpl_comp", "LON_LAT", & 
                "degrees", "cyclic", gsmap_ax, dom_ax, decomp_name="decomp_atm_grid_at_cpl_comp", decomp_id=cplatm_decomp_id, annotation="register atm grid in cpl")
        end if
        call mct_CCPL_register_fields(a2x_ax(eai), cplatm_decomp_id, cplatm_grid_id, 0, "register a2x_ax")
        call mct_CCPL_register_fields(x2a_ax(eai), cplatm_decomp_id, cplatm_grid_id, 1, "register x2a_ax")
        write(cTemp, '(i4)') eai
        call mct_CCPL_register_export_interface(mapper_Cx2a(eai), x2a_ax(eai), cplatm_timer_id(eai), comp_id=cpl_comp_id, mapper_name="mapper_Cx2a", avect_name="x2a_ax"//trim(adjustl(cTemp)), annotation="register export interface in cpl")
        call mct_CCPL_register_import_interface(mapper_Ca2x(eai), a2x_ax(eai), cplatm_timer_id(eai), 0, comp_id=cpl_comp_id, mapper_name="mapper_Ca2x", avect_name="a2x_ax"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
        call mct_CCPL_register_import_interface(mapper_Ca2x(eai), x2a_ax(eai), cplatm_timer_id(eai), 0, comp_id=cpl_comp_id, mapper_name="mapper_Ca2x", avect_name="x2a_ax"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
    end if
    if (trim(aoflux_grid) == 'atm' .and. iamin_CPLID .and. ocn_present) then
        xaoatm_grid_id = mct_CCPL_register_H2D_grid_via_local_data(xao_comp_id, "atm_grid_at_xao_comp", "LON_LAT", & 
           "degrees", "cyclic", gsmap_ax, dom_ax, decomp_name="decomp_atm_grid_at_xao_comp", decomp_id=xaoatm_decomp_id, annotation="register atm grid in xao")
    end if
    if (iamin_CPLATMID(eai)) then
        comps_full_names(1) = "cesm@"//trim(atm_name(eai))
        comps_full_names(2) = "cesm@cpl"
        call CCPL_do_external_coupling_generation(2, comps_full_names)
    endif
end subroutine use_ccpl_register_atm_grid

subroutine use_ccpl_register_lnd_grid(eli)
    implicit none
    integer, intent(in) :: eli
    if (iamin_LNDID(eli)) then
        lnd_grid_id(eli) = CCPL_register_H2D_grid_from_another_component(lnd_comp_id(eli), "lnd_grid_at_lnd_comp", "register lnd grid from CCPL_register_H2D_grid_from_another_component")
        lnd_decomp_id(eli) = mct_CCPL_register_parallel_decomp(lnd_grid_id(eli), "decomp_lnd_grid_at_lnd_comp", gsmap_ll(eli), annotation="register lnd decomp in lnd")
        call mct_CCPL_register_fields(l2x_ll(eli), lnd_decomp_id(eli), lnd_grid_id(eli), 0, "register l2x_ll")
        call mct_CCPL_register_fields(x2l_ll(eli), lnd_decomp_id(eli), lnd_grid_id(eli), 1, "register x2l_ll")
        write(cTemp, '(i4)') eli
        call mct_CCPL_register_import_interface(mapper_Cx2l(eli), x2l_ll(eli), lnd_timer_id(eli), 0, comp_id=lnd_comp_id(eli), mapper_name="mapper_Cx2l", avect_name="x2l_ll"//trim(adjustl(cTemp)), annotation="register import interface in lnd")
        call mct_CCPL_register_export_interface(mapper_Cl2x(eli), l2x_ll(eli), lnd_timer_id(eli), comp_id=lnd_comp_id(eli), mapper_name="mapper_Cl2x", avect_name="l2x_ll"//trim(adjustl(cTemp)), annotation="register export interface in lnd")
        call mct_CCPL_register_export_interface(mapper_Cl2x(eli), x2l_ll(eli), lnd_timer_id(eli), comp_id=lnd_comp_id(eli), mapper_name="mapper_Cl2x", avect_name="x2l_ll"//trim(adjustl(cTemp)), annotation="register export interface in lnd")
    end if
    if (iamin_CPLID) then
        if (eli == 1) then
            cpllnd_grid_id = CCPL_register_H2D_grid_from_another_component(cpl_comp_id, "lnd_grid_at_cpl_comp", "register lnd grid from CCPL_register_H2D_grid_from_another_component")
            cpllnd_decomp_id = mct_CCPL_register_parallel_decomp(cpllnd_grid_id, "decomp_lnd_grid_at_cpl_comp", gsmap_lx, annotation="register lnd decomp in cpl")
        end if
        call mct_CCPL_register_fields(l2x_lx(eli), cpllnd_decomp_id, cpllnd_grid_id, 2, "register l2x_lx")
        call mct_CCPL_register_fields(x2l_lx(eli), cpllnd_decomp_id, cpllnd_grid_id, 3, "register x2l_lx")
        write(cTemp, '(i4)') eli
        call mct_CCPL_register_export_interface(mapper_Cx2l(eli), x2l_lx(eli), cpllnd_timer_id(eli), comp_id=cpl_comp_id, mapper_name="mapper_Cx2l", avect_name="x2l_lx"//trim(adjustl(cTemp)), annotation="register export interface in cpl")
        call mct_CCPL_register_import_interface(mapper_Cl2x(eli), l2x_lx(eli), cpllnd_timer_id(eli), 0, comp_id=cpl_comp_id, mapper_name="mapper_Cl2x", avect_name="l2x_lx"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
        call mct_CCPL_register_import_interface(mapper_Cl2x(eli), x2l_lx(eli), cpllnd_timer_id(eli), 0, comp_id=cpl_comp_id, mapper_name="mapper_Cl2x", avect_name="x2l_lx"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
    end if
    if (iamin_CPLLNDID(eli)) then
        comps_full_names(1) = "cesm@"//trim(lnd_name(eli))
        comps_full_names(2) = "cesm@cpl"
        call CCPL_do_external_coupling_generation(2, comps_full_names)
    endif
end subroutine use_ccpl_register_lnd_grid

subroutine use_ccpl_register_sno_grid(eli)
    implicit none
    integer, intent(in) :: eli

    if (iamin_LNDID(eli)) then
        sno_grid_id(eli) = mct_CCPL_register_H2D_grid_via_local_data(lnd_comp_id(eli), "sno_grid_at_lnd_comp", "LON_LAT", "degrees",&
            "cyclic", gsmap_ss(eli), dom_ss(eli), decomp_name="decomp_sno_grid_at_lnd_comp", decomp_id=sno_decomp_id(eli), annotation="register sno grid in sno")
        call mct_CCPL_register_fields(x2s_ss(eli), sno_decomp_id(eli), sno_grid_id(eli), 0, "register x2s_ss")
        call mct_CCPL_register_fields(s2x_ss(eli), sno_decomp_id(eli), sno_grid_id(eli), 1, "register s2x_ss")
    end if
    if (iamin_CPLID) then
        if (eli == 1) then
           cplsno_grid_id = mct_CCPL_register_H2D_grid_via_local_data(cpl_comp_id, "sno_grid_at_cpl_comp", "LON_LAT", & 
               "degrees", "cyclic", gsmap_sx, dom_sx, decomp_name="decomp_sno_grid_at_cpl_comp", decomp_id=cplsno_decomp_id, annotation="register sno grid in cpl")
        end if
        call mct_CCPL_register_fields(x2s_sx(eli), cplsno_decomp_id, cplsno_grid_id, 4, "register x2s_sx")
        call mct_CCPL_register_fields(s2x_sx(eli), cplsno_decomp_id, cplsno_grid_id, 5, "register s2x_sx")
    end if
end subroutine use_ccpl_register_sno_grid

subroutine use_ccpl_register_rof_grid(eri)
    implicit none
    integer, intent(in) :: eri
    
    if (iamin_ROFID(eri)) then
        rof_grid_id(eri) = mct_CCPL_register_H2D_grid_via_local_data(rof_comp_id(eri), "rof_grid_at_rof_comp", "LON_LAT", "degrees",&
            "cyclic", gsmap_rr(eri), dom_rr(eri), decomp_name="decomp_rof_grid_at_rof_comp", decomp_id=rof_decomp_id(eri), annotation="register rof grid in rof")
        call mct_CCPL_register_fields(x2r_rr(eri), rof_decomp_id(eri), rof_grid_id(eri), 0, "register x2r_rr")
        call mct_CCPL_register_fields(r2x_rr(eri), rof_decomp_id(eri), rof_grid_id(eri), 1, "register r2x_rr")
        write(cTemp, '(i4)') eri
        call mct_CCPL_register_import_interface(mapper_Cx2r(eri), x2r_rr(eri), rof_timer_id(eri), 0, comp_id=rof_comp_id(eri), mapper_name="mapper_Cx2r", avect_name="x2r_rr"//trim(adjustl(cTemp)), annotation="register import interface in rof")
        call mct_CCPL_register_export_interface(mapper_Cr2x(eri), r2x_rr(eri), rof_timer_id(eri), comp_id=rof_comp_id(eri), mapper_name="mapper_Cr2x", avect_name="r2x_rr"//trim(adjustl(cTemp)), annotation="register export interface in rof")
        call mct_CCPL_register_export_interface(mapper_Cr2x(eri), x2r_rr(eri), rof_timer_id(eri), comp_id=rof_comp_id(eri), mapper_name="mapper_Cr2x", avect_name="x2r_rr"//trim(adjustl(cTemp)), annotation="register export interface in rof")
    end if
    if (iamin_CPLID) then
        if (eri == 1) then
            cplrof_grid_id = mct_CCPL_register_H2D_grid_via_local_data(cpl_comp_id, "rof_grid_at_cpl_comp", "LON_LAT", & 
                "degrees", "cyclic", gsmap_rx, dom_rx, decomp_name="decomp_rof_grid_at_cpl_comp", decomp_id=cplrof_decomp_id, annotation="register rof grid in cpl")
        end if
        call mct_CCPL_register_fields(x2r_rx(eri), cplrof_decomp_id, cplrof_grid_id, 6, "register x2r_rx")
        call mct_CCPL_register_fields(r2x_rx(eri), cplrof_decomp_id, cplrof_grid_id, 7, "register r2x_rx")
        write(cTemp, '(i4)') eri
        call mct_CCPL_register_export_interface(mapper_Cx2r(eri), x2r_rx(eri), cplrof_timer_id(eri), comp_id=cpl_comp_id, mapper_name="mapper_Cx2r", avect_name="x2r_rx"//trim(adjustl(cTemp)), annotation="register export interface in cpl")
        call mct_CCPL_register_import_interface(mapper_Cr2x(eri), r2x_rx(eri), cplrof_timer_id(eri), 0, comp_id=cpl_comp_id, mapper_name="mapper_Cr2x", avect_name="r2x_rx"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
        call mct_CCPL_register_import_interface(mapper_Cr2x(eri), x2r_rx(eri), cplrof_timer_id(eri), 0, comp_id=cpl_comp_id, mapper_name="mapper_Cr2x", avect_name="x2r_rx"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
    end if
    if (iamin_CPLROFID(eri)) then
        comps_full_names(1) = "cesm@"//trim(rof_name(eri))
        comps_full_names(2) = "cesm@cpl"
        call CCPL_do_external_coupling_generation(2, comps_full_names)
    endif
end subroutine use_ccpl_register_rof_grid

subroutine use_ccpl_register_ocn_grid(eoi)
    implicit none
    integer, intent(in) :: eoi
    
    if (iamin_OCNID(eoi)) then
        !ocn_grid_id(eoi) = CCPL_register_H2D_grid_from_another_component(ocn_comp_id(eoi), "ocn_grid_at_ocn_comp", "register ocn grid from CCPL_register_H2D_grid_from_another_component")
        !ocn_decomp_id(eoi) = mct_CCPL_register_parallel_decomp(ocn_grid_id(eoi), "decomp_ocn_grid_at_ocn_comp", gsmap_oo(eoi), annotation="register ocn decomp in ocn")
        ocn_grid_id(eoi) = mct_CCPL_register_H2D_grid_via_local_data(ocn_comp_id(eoi), "ocn_grid_at_ocn_comp", "LON_LAT", "degrees",&
            "cyclic", gsmap_oo(eoi), dom_oo(eoi), decomp_name="decomp_ocn_grid_at_ocn_comp", decomp_id=ocn_decomp_id(eoi), annotation="register ocn grid in ocn")
        call mct_CCPL_register_fields(o2x_oo(eoi), ocn_decomp_id(eoi), ocn_grid_id(eoi), 0, "register o2x_oo")
        call mct_CCPL_register_fields(x2o_oo(eoi), ocn_decomp_id(eoi), ocn_grid_id(eoi), 1, "register x2o_oo")
        write(cTemp, '(i4)') eoi
        call mct_CCPL_register_import_interface(mapper_Cx2o(eoi), x2o_oo(eoi), ocn_Cx2o_timer_id(eoi), 0, comp_id=ocn_comp_id(eoi), mapper_name="mapper_Cx2o", avect_name="x2o_oo"//trim(adjustl(cTemp)), annotation="register import interface in ocn")
        call mct_CCPL_register_export_interface(mapper_Co2x(eoi), o2x_oo(eoi), ocn_Co2x_timer_id(eoi), comp_id=ocn_comp_id(eoi), mapper_name="mapper_Co2x", avect_name="o2x_oo"//trim(adjustl(cTemp)), annotation="register export interface in ocn")
        call mct_CCPL_register_export_interface(mapper_Co2x(eoi), x2o_oo(eoi), ocn_Co2x_timer_id(eoi), comp_id=ocn_comp_id(eoi), mapper_name="mapper_Co2x", avect_name="x2o_oo"//trim(adjustl(cTemp)), annotation="register export interface in ocn")
    end if
    if (iamin_CPLID) then
        if (eoi == 1) then
            !cplocn_grid_id = CCPL_register_H2D_grid_from_another_component(cpl_comp_id, "ocn_grid_at_cpl_comp", "register ocn grid from CCPL_register_H2D_grid_from_another_component")
            !cplocn_decomp_id = mct_CCPL_register_parallel_decomp(cplocn_grid_id, "decomp_ocn_grid_at_cpl_comp", gsmap_ox, annotation="register ocn decomp in cpl")
            cplocn_grid_id = mct_CCPL_register_H2D_grid_via_local_data(cpl_comp_id, "ocn_grid_at_cpl_comp", "LON_LAT", & 
                "degrees", "cyclic", gsmap_ox, dom_ox, decomp_name="decomp_ocn_grid_at_cpl_comp", decomp_id=cplocn_decomp_id, annotation="register ocn grid in cpl")
        end if
        call mct_CCPL_register_fields(o2x_ox(eoi), cplocn_decomp_id, cplocn_grid_id, 14, "register o2x_ox")
        call mct_CCPL_register_fields(x2o_ox(eoi), cplocn_decomp_id, cplocn_grid_id, 15, "register x2o_ox")
        call mct_CCPL_register_import_interface(mapper_Co2x(eoi), o2x_ox(eoi), cplocn_Co2x_timer_id(eoi), 0, comp_id=cpl_comp_id, mapper_name="mapper_Co2x", avect_name="o2x_ox"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
        call mct_CCPL_register_import_interface(mapper_Co2x(eoi), x2o_ox(eoi), cplocn_Co2x_timer_id(eoi), 0, comp_id=cpl_comp_id, mapper_name="mapper_Co2x", avect_name="x2o_ox"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
    end if
    if (trim(aoflux_grid) == 'atm' .and. iamin_CPLID .and. ocn_present) then
        !xaoocn_grid_id = CCPL_register_H2D_grid_from_another_component(xao_comp_id, "ocn_grid_at_xao_comp", "register ocn grid from CCPL_register_H2D_grid_from_another_component")
        !xaoocn_decomp_id = mct_CCPL_register_parallel_decomp(xaoocn_grid_id, "decomp_ocn_grid_at_xao_comp", gsmap_ox, annotation="register ocn decomp in xao")
        xaoocn_grid_id = mct_CCPL_register_H2D_grid_via_local_data(xao_comp_id, "ocn_grid_at_xao_comp", "LON_LAT", & 
            "degrees", "cyclic", gsmap_ox, dom_ox, decomp_name="decomp_ocn_grid_at_xao_comp", decomp_id=xaoocn_decomp_id, annotation="register ocn grid in xao")
    end if
    if (iamin_CPLOCNID(eoi)) then
        comps_full_names(1) = "cesm@"//trim(ocn_name(eoi))
        comps_full_names(2) = "cesm@cpl"
        call CCPL_do_external_coupling_generation(2, comps_full_names)
    endif
end subroutine use_ccpl_register_ocn_grid

subroutine use_ccpl_register_ice_grid(eii)
    implicit none
    integer, intent(in) :: eii

    if (iamin_ICEID(eii)) then
        ice_grid_id(eii) = CCPL_register_H2D_grid_from_another_component(ice_comp_id(eii), "ice_grid_at_ice_comp", "register ice grid from CCPL_register_H2D_grid_from_another_component")
        ice_decomp_id(eii) = mct_CCPL_register_parallel_decomp(ice_grid_id(eii), "decomp_ice_grid_at_ice_comp", gsmap_ii(eii), annotation="register ice decomp in ice")
        !ice_grid_id(eii) = mct_CCPL_register_H2D_grid_via_local_data(ice_comp_id(eii), "ice_grid_at_ice_comp", "LON_LAT", "degrees",&
        !    "cyclic", gsmap_ii(eii), dom_ii(eii), decomp_name="decomp_ice_grid_at_ice_comp", decomp_id=ice_decomp_id(eii), annotation="register ice grid in ice")
        call mct_CCPL_register_fields(x2i_ii(eii), ice_decomp_id(eii), ice_grid_id(eii), 0, "register x2i_ii")
        call mct_CCPL_register_fields(i2x_ii(eii), ice_decomp_id(eii), ice_grid_id(eii), 1, "register i2x_ii")
        write(cTemp, '(i4)') eii
        call mct_CCPL_register_import_interface(mapper_Cx2i(eii), x2i_ii(eii), ice_timer_id(eii), 0, comp_id=ice_comp_id(eii), mapper_name="mapper_Cx2i", avect_name="x2i_ii"//trim(adjustl(cTemp)), annotation="register import interface in ice")
        call mct_CCPL_register_export_interface(mapper_Ci2x(eii), i2x_ii(eii), ice_timer_id(eii), comp_id=ice_comp_id(eii), mapper_name="mapper_Ci2x", avect_name="i2x_ii"//trim(adjustl(cTemp)), annotation="register export interface in ice")
        call mct_CCPL_register_export_interface(mapper_Ci2x(eii), x2i_ii(eii), ice_timer_id(eii), comp_id=ice_comp_id(eii), mapper_name="mapper_Ci2x", avect_name="x2i_ii"//trim(adjustl(cTemp)), annotation="register export interface in ice")
    end if
    if (iamin_CPLID) then
        if (eii == 1) then
            cplice_grid_id = CCPL_register_H2D_grid_from_another_component(cpl_comp_id, "ice_grid_at_cpl_comp", "register ice grid from CCPL_register_H2D_grid_from_another_component")
            cplice_decomp_id = mct_CCPL_register_parallel_decomp(cplice_grid_id, "decomp_ice_grid_at_cpl_comp", gsmap_ix, annotation="register ice decomp in cpl")
            !cplice_grid_id = mct_CCPL_register_H2D_grid_via_local_data(cpl_comp_id, "ice_grid_at_cpl_comp", "LON_LAT", & 
            !    "degrees", "cyclic", gsmap_ix, dom_ix, decomp_name="decomp_ice_grid_at_cpl_comp", decomp_id=cplice_decomp_id, annotation="register ice grid in cpl")
        end if
        call mct_CCPL_register_fields(x2i_ix(eii), cplice_decomp_id, cplice_grid_id, 8, "register x2i_ix")
        call mct_CCPL_register_fields(i2x_ix(eii), cplice_decomp_id, cplice_grid_id, 9, "register i2x_ix")
        write(cTemp, '(i4)') eii
        call mct_CCPL_register_export_interface(mapper_Cx2i(eii), x2i_ix(eii), cplice_timer_id(eii), comp_id=cpl_comp_id, mapper_name="mapper_Cx2i", avect_name="x2i_ix"//trim(adjustl(cTemp)), annotation="register export interface in cpl")
        call mct_CCPL_register_import_interface(mapper_Ci2x(eii), i2x_ix(eii), cplice_timer_id(eii), 0, comp_id=cpl_comp_id, mapper_name="mapper_Ci2x", avect_name="i2x_ix"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
        call mct_CCPL_register_import_interface(mapper_Ci2x(eii), x2i_ix(eii), cplice_timer_id(eii), 0, comp_id=cpl_comp_id, mapper_name="mapper_Ci2x", avect_name="x2i_ix"//trim(adjustl(cTemp)), annotation="register import interface in cpl")
    end if
    if (iamin_CPLICEID(eii)) then
        comps_full_names(1) = "cesm@"//trim(ice_name(eii))
        comps_full_names(2) = "cesm@cpl"
        call CCPL_do_external_coupling_generation(2, comps_full_names)
    endif
end subroutine use_ccpl_register_ice_grid

subroutine use_ccpl_register_wav_grid(ewi)
    implicit none
    integer, intent(in) :: ewi

    if (iamin_WAVID(ewi)) then
        wav_grid_id(ewi) = mct_CCPL_register_H2D_grid_via_local_data(wav_comp_id(ewi), "wav_grid_at_wav_comp", "LON_LAT", "degrees",&
            "cyclic", gsmap_ww(ewi), dom_ww(ewi), decomp_name="decomp_wav_grid_at_wav_comp", decomp_id=wav_decomp_id(ewi), annotation="register wav grid in wav")
        call mct_CCPL_register_fields(x2w_ww(ewi), wav_decomp_id(ewi), wav_grid_id(ewi), 0, "register x2w_ww")
        call mct_CCPL_register_fields(w2x_ww(ewi), wav_decomp_id(ewi), wav_grid_id(ewi), 1, "register w2x_ww")
    end if
    if (iamin_CPLID) then
        if (ewi == 1) then
            cplwav_grid_id = mct_CCPL_register_H2D_grid_via_local_data(cpl_comp_id, "wav_grid_at_cpl_comp", "LON_LAT", & 
                "degrees", "cyclic", gsmap_wx, dom_wx, decomp_name="decomp_wav_grid_at_cpl_comp", decomp_id=cplwav_decomp_id, annotation="register wav grid in cpl")
        end if
        call mct_CCPL_register_fields(x2w_wx(ewi), cplwav_decomp_id, cplwav_grid_id, 12, "register x2w_wx")
        call mct_CCPL_register_fields(w2x_wx(ewi), cplwav_decomp_id, cplwav_grid_id, 13, "register w2x_wx")
    end if
end subroutine use_ccpl_register_wav_grid

subroutine use_ccpl_register_cpl_fields()
    do eai = 1,num_inst_atm
        if (lnd_present) call mct_CCPL_register_fields(a2x_lx(eai), cpllnd_decomp_id, cpllnd_grid_id, 16, "register a2x_lx")
        if (ocn_present) call mct_CCPL_register_fields(a2x_ox(eai), cplocn_decomp_id, cplocn_grid_id, 17, "register a2x_ox")
        if (ice_present) call mct_CCPL_register_fields(a2x_ix(eai), cplice_decomp_id, cplice_grid_id, 18, "register a2x_ix")
        if (wav_present) call mct_CCPL_register_fields(a2x_wx(eai), cplwav_decomp_id, cplwav_grid_id, 19, "register a2x_wx")
    enddo

    do eli = 1,num_inst_lnd
        if (atm_present) call mct_CCPL_register_fields(l2x_ax(eli), cplatm_decomp_id, cplatm_grid_id, 20, "register l2x_ax")
        if (glc_present) call mct_CCPL_register_fields(s2x_gx(eli), cplglc_decomp_id, cplglc_grid_id, 21, "register s2x_gx")
        if (lnd_present) call mct_CCPL_register_fields(x2racc_lx(eli), cpllnd_decomp_id, cpllnd_grid_id, 22, "register x2racc_lx")
    enddo
    if (rof_present) call mct_CCPL_register_fields(x2r_rx_tmp, cplrof_decomp_id, cplrof_grid_id, 23, "register x2r_rx_tmp")

    do eri = 1,num_inst_rof
        if (rof_present) call mct_CCPL_register_fields(r2xacc_rx(eri), cplrof_decomp_id, cplrof_grid_id, 24, "register x2xacc_rx")
        if (ocn_present) call mct_CCPL_register_fields(r2x_ox(eri), cplocn_decomp_id, cplocn_grid_id, 25, "register r2x_ox")
        if (lnd_present) call mct_CCPL_register_fields(r2x_lx(eri), cpllnd_decomp_id, cpllnd_grid_id, 26, "register r2x_lx")
    enddo

    do eoi = 1,num_inst_ocn
        if (atm_present) call mct_CCPL_register_fields(o2x_ax(eoi), cplatm_decomp_id, cplatm_grid_id, 27, "register o2x_ax")
        if (ice_present) call mct_CCPL_register_fields(o2x_ix(eoi), cplice_decomp_id, cplice_grid_id, 28, "register o2x_ix")
        if (wav_present) call mct_CCPL_register_fields(o2x_wx(eoi), cplwav_decomp_id, cplwav_grid_id, 29, "register o2x_wx")
    enddo

    do eii = 1,num_inst_ice
        if (ocn_present) call mct_CCPL_register_fields(i2x_ox(eii), cplocn_decomp_id, cplocn_grid_id, 30, "register i2x_ox")
        if (wav_present) call mct_CCPL_register_fields(i2x_wx(eii), cplwav_decomp_id, cplwav_grid_id, 31, "register i2x_wx")
        if (atm_present) call mct_CCPL_register_fields(i2x_ax(eii), cplatm_decomp_id, cplatm_grid_id, 32, "register i2x_ax")
    enddo

    do egi = 1,num_inst_glc
        if (sno_present) call mct_CCPL_register_fields(g2x_sx(egi), cplsno_decomp_id, cplsno_grid_id, 33, "register g2x_sx")
    enddo

    do ewi = 1,num_inst_wav
        if (ocn_present) call mct_CCPL_register_fields(w2x_ox(ewi), cplocn_decomp_id, cplocn_grid_id, 34, "register w2x_ox")
    enddo

    do exi = 1,num_inst_xao
        if (trim(aoflux_grid) /= 'atm') then
            if (atm_present) call mct_CCPL_register_fields(xao_ax(exi), cplatm_decomp_id, cplatm_grid_id, 35, "register xao_ax")
            if (ocn_present) call mct_CCPL_register_fields(xao_ox(exi), cplocn_decomp_id, cplocn_grid_id, 36, "register xao_ox")
        else
            if (atm_present) call mct_CCPL_register_fields(xao_ax(exi), xaoatm_decomp_id, xaoatm_grid_id, 35, "register xao_ax")
            if (ocn_present) call mct_CCPL_register_fields(xao_ox(exi), xaoocn_decomp_id, xaoocn_grid_id, 36, "register xao_ox")
        end if
    enddo
end subroutine use_ccpl_register_cpl_fields

subroutine use_ccpl_register_cpl_interface()
    if (ocn_present) then
        do eai = 1, num_inst_atm
            write(cTemp, '(i4)') eai
            call mct_CCPL_register_frac_based_remap_interface(mapper_Sa2o, a2x_ax(eai), a2x_ox(eai), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Sa2o", av_s_name="a2x_ax"//trim(adjustl(cTemp)), av_d_name="a2x_ox"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_a2x_states_no_uv, annotation="register state remap from atm to ocn")
            call mct_CCPL_register_frac_based_remap_interface(mapper_Fa2o, a2x_ax(eai), a2x_ox(eai), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Fa2o", av_s_name="a2x_ax"//trim(adjustl(cTemp)), av_d_name="a2x_ox"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_a2x_fluxes, annotation="register flux remap from atm to ocn")
            call mct_CCPL_register_frac_based_remap_interface(mapper_Va2o, a2x_ax(eai), a2x_ox(eai), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Va2o", av_s_name="a2x_ax"//trim(adjustl(cTemp)), av_d_name="a2x_ox"//trim(adjustl(cTemp)), &
                fldlist="Sa_u:Sa_v", annotation="register flux remap from atm to ocn")
        end do
    
        do eoi = 1, num_inst_ocn
            write(cTemp, '(i4)') eoi
            call mct_CCPL_register_frac_based_remap_interface(mapper_So2a, o2x_ox(eoi), o2x_ax(eoi), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_So2a", av_s_name="o2x_ox"//trim(adjustl(cTemp)), av_d_name="o2x_ax"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_o2x_states, annotation="register state remap from ocn to atm")
            call mct_CCPL_register_frac_based_remap_interface(mapper_Fo2a, o2x_ox(eoi), o2x_ax(eoi), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Fo2a", av_s_name="o2x_ox"//trim(adjustl(cTemp)), av_d_name="o2x_ax"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_o2x_fluxes, annotation="register flux remap from ocn to atm")
        end do
    
        if (trim(aoflux_grid) == 'ocn') then
            do exi = 1, num_inst_xao
                write(cTemp, '(i4)') exi
                call mct_CCPL_register_frac_based_remap_interface(mapper_Fo2a, xao_ox(exi), xao_ax(exi), cpl_timer_id, 0, cpl_comp_id, &
                    mapper_name="mapper_Fo2a", av_s_name="xao_ox"//trim(adjustl(cTemp)), av_d_name="xao_ax"//trim(adjustl(cTemp)), &
                    fldlist=seq_flds_xao_fluxes, annotation="register xao flux remap from ocn to atm")
                call mct_CCPL_register_frac_based_remap_interface(mapper_So2a, xao_ox(exi), xao_ax(exi), cpl_timer_id, 0, cpl_comp_id, &
                    mapper_name="mapper_So2a", av_s_name="xao_ox"//trim(adjustl(cTemp)), av_d_name="xao_ax"//trim(adjustl(cTemp)), &
                    fldlist=seq_flds_xao_states, stage="states", annotation="register xao flux remap from ocn to atm")
            end do
        end if
        ...
        if (trim(aoflux_grid) == 'atm') then
            do exi = 1, num_inst_xao
                write(cTemp, '(i4)') exi
                call mct_CCPL_register_frac_based_remap_interface(mapper_Fa2o, xao_ax(exi), xao_ox(exi), xao_timer_id, 0, xao_comp_id, &
                    mapper_name="mapper_Fa2o", av_s_name="xao_ax"//trim(adjustl(cTemp)), av_d_name="xao_ox"//trim(adjustl(cTemp)), &
                    annotation="register xao flux remap from atm to ocn")
                call mct_CCPL_register_frac_based_remap_interface(mapper_So2a, xao_ox(exi), xao_ax(exi), xao_timer_id, 0, xao_comp_id, &
                    mapper_name="mapper_So2a", av_s_name="xao_ox"//trim(adjustl(cTemp)), av_d_name="xao_ax"//trim(adjustl(cTemp)), &
                    fldlist=seq_flds_xao_albedo, stage="albedo", annotation="register xao flux remap from ocn to atm")
            end do
        else
            do exi = 1, num_inst_xao
                write(cTemp, '(i4)') exi
    ...
    if (ice_present .and. ocn_present) then
        do eii = 1, num_inst_ice
            write(cTemp, '(i4)') eii
            call mct_CCPL_register_frac_based_remap_interface(mapper_SFi2o, i2x_ix(eii), i2x_ox(eii), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_SFi2o", av_s_name="i2x_ix"//trim(adjustl(cTemp)), av_d_name="i2x_ox"//trim(adjustl(cTemp)), &
                annotation="register state remap from ice to ocn")
        end do
        do eoi = 1, num_inst_ocn
            write(cTemp, '(i4)') eoi
            call mct_CCPL_register_frac_based_remap_interface(mapper_SFo2i, o2x_ox(eoi), o2x_ix(eoi), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_SFo2i", av_s_name="o2x_ox"//trim(adjustl(cTemp)), av_d_name="o2x_ix"//trim(adjustl(cTemp)), &
                annotation="register state remap from ocn to ice")
        end do
        do eai = 1, num_inst_atm
            write(cTemp, '(i4)') eai
            call mct_CCPL_register_frac_based_remap_interface(mapper_SFo2i, a2x_ox(eai), a2x_ix(eai), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_SFo2i", av_s_name="a2x_ox"//trim(adjustl(cTemp)), av_d_name="a2x_ix"//trim(adjustl(cTemp)), &
                annotation="register state remap from ice to atm")
        end do
    endif
    if (ice_present) then
        do eii = 1, num_inst_ice
            write(cTemp, '(i4)') eii
            call mct_CCPL_register_frac_based_remap_interface(mapper_Si2a, i2x_ix(eii), i2x_ax(eii), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Si2a", av_s_name="i2x_ix"//trim(adjustl(cTemp)), av_d_name="i2x_ax"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_i2x_states, annotation="register state remap from ice to atm")
            call mct_CCPL_register_frac_based_remap_interface(mapper_Fi2a, i2x_ix(eii), i2x_ax(eii), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Fi2a", av_s_name="i2x_ix"//trim(adjustl(cTemp)), av_d_name="i2x_ax"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_i2x_fluxes, annotation="register state remap from ice to atm")
        end do
    endif
    if (rof_present .and. ocnrof_prognostic) then
        if (flood_present) then
            do eri = 1, num_inst_rof
                write(cTemp, '(i4)') eri
                call mct_CCPL_register_frac_based_remap_interface(mapper_Fr2o, r2xacc_rx(eri), r2x_ox(eri), cpl_timer_id, 0, cpl_comp_id, &
                    mapper_name="mapper_Fr2o", av_s_name="r2xacc_rx"//trim(adjustl(cTemp)), av_d_name="r2x_ox"//trim(adjustl(cTemp)), &
                    fldlist='Flrr_flood', annotation="register remap from rof to ocn")
            end do
        endif
        do eri = 1, num_inst_rof
            write(cTemp, '(i4)') eri
            eoi = mod(eri-1, num_inst_ocn) + 1
            call mct_CCPL_register_normal_remap_interface(mapper_Rr2o, r2xacc_rx(eri), r2x_ox(eri), cplocn_Cx2o_timer_id(eri), 0, cpl_comp_id, &
                mapper_name="mapper_Rr2o", av_s_name="r2xacc_rx"//trim(adjustl(cTemp)), av_d_name="r2x_ox"//trim(adjustl(cTemp)), &
                fldlist='Forr_roff:Forr_ioff', annotation="register flood remap from rof to ocn")
        end do
    endif
    if (rof_present .and. lnd_present ) then ! TODO - land and rof might be on the same grid (see below)
        do eri = 1, num_inst_rof
            write(cTemp, '(i4)') eri
            if (flood_present) call mct_CCPL_register_frac_based_remap_interface(mapper_Fr2l, r2x_rx(eri), r2x_lx(eri), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Fr2l", av_s_name="r2x_rx"//trim(adjustl(cTemp)), av_d_name="r2x_lx"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_r2x_fluxes, annotation="register flux remap from rof to lnd")
            if (rof_present) call mct_CCPL_register_frac_based_remap_interface(mapper_Sr2l, r2x_rx(eri), r2x_lx(eri), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Sr2l", av_s_name="r2x_rx"//trim(adjustl(cTemp)), av_d_name="r2x_lx"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_r2x_states, annotation="register state remap from rof to lnd")
        end do

        do eli = 1, num_inst_lnd
            eri = mod((eli-1),num_inst_rof) + 1
            write(cTemp, '(i4)') eli
            call mct_CCPL_register_frac_based_remap_interface(mapper_Fl2r, x2racc_lx(eli), x2r_rx_tmp, cplrof_timer_id(eri), 0, cpl_comp_id, &
                mapper_name="mapper_Fl2r", av_s_name="x2racc_lx"//trim(adjustl(cTemp)), av_d_name="x2r_rx_tmp", &
                fldlist=seq_flds_x2r_fluxes, annotation="register state remap from lnd to rof")
        end do
    end if
    if (lnd_present) then
        do eai = 1, num_inst_atm
            write(cTemp, '(i4)') eai
            call mct_CCPL_register_frac_based_remap_interface(mapper_Fa2l, a2x_ax(eai), a2x_lx(eai), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Fa2l", av_s_name="a2x_ax"//trim(adjustl(cTemp)), av_d_name="a2x_lx"//trim(adjustl(cTemp)), &
                annotation="register remap from atm to lnd")
        end do
        do eli = 1, num_inst_lnd
            write(cTemp, '(i4)') eli
            call mct_CCPL_register_frac_based_remap_interface(mapper_Fl2a, l2x_lx(eli), l2x_ax(eli), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Fl2a", av_s_name="l2x_lx"//trim(adjustl(cTemp)), av_d_name="l2x_ax"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_l2x_fluxes, annotation="register flux remap from atm to lnd")
            call mct_CCPL_register_frac_based_remap_interface(mapper_Sl2a, l2x_lx(eli), l2x_ax(eli), cpl_timer_id, 0, cpl_comp_id, &
                mapper_name="mapper_Sl2a", av_s_name="l2x_lx"//trim(adjustl(cTemp)), av_d_name="l2x_ax"//trim(adjustl(cTemp)), &
                fldlist=seq_flds_l2x_states, annotation="register states remap from atm to lnd")
        end do
    endif
end subroutine use_ccpl_register_cpl_interface

subroutine use_ccpl_end_coupling_configuration()
    if (trim(aoflux_grid) == 'atm' .and. iamin_CPLID .and. ocn_present) then
        !call CCPL_do_family_coupling_generation(xao_comp_id)
        call CCPL_end_coupling_configuration(xao_comp_id, annotation="component xao end registration")
    end if
    do ewi = 1, num_inst_wav
        if (iamin_WAVID(ewi) .and. wav_present) then
        !    call CCPL_do_family_coupling_generation(wav_comp_id(ewi))
            call CCPL_end_coupling_configuration(wav_comp_id(ewi), annotation="component "//trim(wav_name(ewi))//" end registration")
        end if
    end do
    do eri = 1, num_inst_rof
        if (iamin_ROFID(eri) .and. rof_present) then
            !call CCPL_do_family_coupling_generation(rof_comp_id(eri))
            call CCPL_end_coupling_configuration(rof_comp_id(eri), annotation="component "//trim(rof_name(eri))//" end registration")
        end if
    end do
    do egi = 1, num_inst_glc
        if (iamin_GLCID(egi) .and. glc_present) then
            !call CCPL_do_family_coupling_generation(glc_comp_id(egi))
            call CCPL_end_coupling_configuration(glc_comp_id(egi), annotation="component "//trim(glc_name(egi))//" end registration")
        end if
    end do
    do eoi = 1, num_inst_ocn
        if (iamin_OCNID(eoi) .and. ocn_present) then
            !call CCPL_do_family_coupling_generation(ocn_comp_id(eoi))
            call CCPL_end_coupling_configuration(ocn_comp_id(eoi), annotation="component "//trim(ocn_name(eoi))//" end registration")
        end if
    end do
    do eii = 1, num_inst_ice
        if (iamin_ICEID(eii) .and. ice_present) then
            !call CCPL_do_family_coupling_generation(ice_comp_id(eii))
            call CCPL_end_coupling_configuration(ice_comp_id(eii), annotation="component "//trim(ice_name(eii))//" end registration")
        end if
    end do
    do eli = 1, num_inst_lnd
        if (iamin_LNDID(eli) .and. lnd_present) then
            !call CCPL_do_family_coupling_generation(lnd_comp_id(eli))
            call CCPL_end_coupling_configuration(lnd_comp_id(eli), annotation="component "//trim(lnd_name(eli))//" end registration")
        end if
    end do
    do eai = 1, num_inst_atm
        if (iamin_ATMID(eai) .and. atm_present) then
            !call CCPL_do_family_coupling_generation(atm_comp_id(eai))
            call CCPL_end_coupling_configuration(atm_comp_id(eai), annotation="component "//trim(atm_name(eai))//" end registration")
        end if
    end do
    if (iamin_CPLID) then
        !call CCPL_do_family_coupling_generation(cpl_comp_id)
        call CCPL_end_coupling_configuration(cpl_comp_id, annotation="component cpl7 end registration")
    end if
    !call CCPL_do_family_coupling_generation(root_comp_id)
    call CCPL_end_coupling_configuration(root_comp_id, annotation="root component cesm end registration")
end subroutine use_ccpl_end_coupling_configuration

subroutine use_ccpl_do_restart_read()
    if (iamin_CPLID) then 
        call CCPL_start_restart_read_IO(cpl_comp_id, annotation="read restart file in CPL")
        call seq_timemgr_EClockGetData( EClock_d, curr_ymd=ymd, curr_tod=tod )
        call CCPL_check_current_time(cpl_comp_id, ymd, tod, annotation="check time for cpl")
        call CCPL_restart_read_fields_all(cpl_comp_id, annotation="read restart fields in CPL")
    endif
    if (trim(aoflux_grid) == 'atm' .and. iamin_CPLID .and. ocn_present) then
        call CCPL_start_restart_read_IO(xao_comp_id, annotation="read restart file in xao")
        call CCPL_restart_read_fields_all(xao_comp_id, annotation="read restart fields in xao")
    endif
    do eai = 1, num_inst_atm
        if (iamin_ATMID(eai) .and. atm_present) then
            call CCPL_start_restart_read_IO(atm_comp_id(eai), annotation="read restart file in "//trim(atm_name(eai)))
            call seq_timemgr_EClockGetData( EClock_a, curr_ymd=ymd, curr_tod=tod )
            call CCPL_check_current_time(atm_comp_id(eai), ymd, tod, annotation="check time for atm")
            call CCPL_restart_read_fields_all(atm_comp_id(eai), annotation="read restart fields in "//trim(atm_name(eai)))
        end if
    end do
    do eli = 1, num_inst_lnd
        if (iamin_LNDID(eli) .and. lnd_present) then
            call CCPL_start_restart_read_IO(lnd_comp_id(eli), annotation="read restart file in "//trim(lnd_name(eli)))
            call seq_timemgr_EClockGetData( EClock_l, curr_ymd=ymd, curr_tod=tod )
            call CCPL_check_current_time(lnd_comp_id(eli), ymd, tod, annotation="check time for lnd")
            call CCPL_restart_read_fields_all(lnd_comp_id(eli), annotation="read restart fields in "//trim(lnd_name(eli)))
        end if
    end do
    do eii = 1, num_inst_ice
        if (iamin_ICEID(eii) .and. ice_present) then
            call CCPL_start_restart_read_IO(ice_comp_id(eii), annotation="read restart file in "//trim(ice_name(eii)))
            call seq_timemgr_EClockGetData( EClock_i, curr_ymd=ymd, curr_tod=tod )
            call CCPL_check_current_time(ice_comp_id(eii), ymd, tod, annotation="check time for ice")
            call CCPL_restart_read_fields_all(ice_comp_id(eii), annotation="read restart fields in "//trim(ice_name(eii)))
        end if
    end do
    do eoi = 1, num_inst_ocn
        if (iamin_OCNID(eoi) .and. ocn_present) then
            call CCPL_start_restart_read_IO(ocn_comp_id(eoi), annotation="read restart file in "//trim(ocn_name(eoi)))
            call seq_timemgr_EClockGetData( EClock_o, curr_ymd=ymd, curr_tod=tod )
            call CCPL_check_current_time(ocn_comp_id(eoi), ymd, tod, annotation="check time for ocn")
            call CCPL_restart_read_fields_all(ocn_comp_id(eoi), annotation="read restart fields in "//trim(ocn_name(eoi)))
        end if
    end do
    do egi = 1, num_inst_glc
        if (iamin_GLCID(egi) .and. glc_present) then
            call CCPL_start_restart_read_IO(glc_comp_id(egi), annotation="read restart file in "//trim(glc_name(egi)))
            call seq_timemgr_EClockGetData( EClock_g, curr_ymd=ymd, curr_tod=tod )
            call CCPL_check_current_time(glc_comp_id(egi), ymd, tod, annotation="check time for glc")
            call CCPL_restart_read_fields_all(glc_comp_id(egi), annotation="read restart fields in "//trim(glc_name(egi)))
        end if
    end do
    do eri = 1, num_inst_rof
        if (iamin_ROFID(eri) .and. rof_present) then
            call CCPL_start_restart_read_IO(rof_comp_id(eri), annotation="read restart file in "//trim(rof_name(eri)))
            call seq_timemgr_EClockGetData(EClock_r, curr_ymd=ymd, curr_tod=tod )
            call CCPL_check_current_time(rof_comp_id(eri), ymd, tod, annotation="check time for rof")
            call CCPL_restart_read_fields_all(rof_comp_id(eri), annotation="read restart fields in "//trim(rof_name(eri)))
        end if
    end do
    do ewi = 1, num_inst_wav
        if (iamin_WAVID(ewi) .and. wav_present) then
            call CCPL_start_restart_read_IO(wav_comp_id(ewi), annotation="read restart file in "//trim(wav_name(ewi)))
            call seq_timemgr_EClockGetData( EClock_w, curr_ymd=ymd, curr_tod=tod )
            call CCPL_check_current_time(wav_comp_id(ewi), ymd, tod, annotation="check time for wav")
            call CCPL_restart_read_fields_all(wav_comp_id(ewi), annotation="read restart fields in "//trim(wav_name(ewi)))
        end if
    end do
end subroutine use_ccpl_do_restart_read

subroutine use_ccpl_advance_time()
    if (wav_present) then
        do ewi = 1, num_inst_wav
            if (iamin_WAVID(ewi)) call CCPL_advance_time(wav_comp_id(ewi))
        end do
    endif

    if (glc_present) then
        do egi = 1, num_inst_glc
            if (iamin_GLCID(egi)) call CCPL_advance_time(glc_comp_id(egi))
        end do
    endif

    if (rof_present) then
        do eri = 1, num_inst_rof
            if (iamin_rofID(eri)) call CCPL_advance_time(rof_comp_id(eri))
        end do
    endif

    if (ice_present) then
        do eii = 1, num_inst_ice
            if (iamin_ICEID(eii)) call CCPL_advance_time(ice_comp_id(eii))
        end do
    endif

    if (ocn_present) then
        do eoi = 1, num_inst_ocn
            if (iamin_OCNID(eoi)) call CCPL_advance_time(ocn_comp_id(eoi))
        end do
    endif

    if (lnd_present) then
        do eli = 1, num_inst_lnd
            if (iamin_LNDID(eli)) call CCPL_advance_time(lnd_comp_id(eli))
        end do
    endif

    if (atm_present) then
        do eai = 1, num_inst_atm
            if (iamin_ATMID(eai)) call CCPL_advance_time(atm_comp_id(eai))
        end do
    endif

    if (iamin_CPLID) then
        call CCPL_advance_time(cpl_comp_id)
    endif

    if (trim(aoflux_grid) == 'atm' .and. iamin_CPLID .and. ocn_present) then
        call CCPL_advance_time(xao_comp_id)
    end if
end subroutine use_ccpl_advance_time

subroutine use_ccpl_do_restart_write()
    if (iamin_CPLID) then 
        call CCPL_do_restart_write_IO(cpl_comp_id, .false., bypass_import_fields=.true., annotation="write restart file in CPL")
    endif
    if (trim(aoflux_grid) == 'atm' .and. iamin_CPLID .and. ocn_present) then
        call CCPL_do_restart_write_IO(xao_comp_id, .false., bypass_import_fields=.true., annotation="write restart file in xao")
    endif
    do eai = 1, num_inst_atm
        if (iamin_ATMID(eai) .and. atm_present) then
            call CCPL_do_restart_write_IO(atm_comp_id(eai), .false., bypass_import_fields=.true., annotation="write restart file in "//trim(atm_name(eai)))
        end if
        end do
    do eli = 1, num_inst_lnd
        if (iamin_LNDID(eli) .and. lnd_present) then
            call CCPL_do_restart_write_IO(lnd_comp_id(eli), .false., bypass_import_fields=.true., annotation="write restart file in "//trim(lnd_name(eli)))
        end if
    end do
    do eii = 1, num_inst_ice
        if (iamin_ICEID(eii) .and. ice_present) then
            call CCPL_do_restart_write_IO(ice_comp_id(eii), .false., bypass_import_fields=.true., annotation="write restart file in "//trim(ice_name(eii)))
        end if
    end do
    do eoi = 1, num_inst_ocn
        if (iamin_OCNID(eoi) .and. ocn_present) then
            call CCPL_do_restart_write_IO(ocn_comp_id(eoi), .false., bypass_import_fields=.true., annotation="write restart file in "//trim(ocn_name(eoi)))
        end if
    end do
    do egi = 1, num_inst_glc
        if (iamin_GLCID(egi) .and. glc_present) then
            call CCPL_do_restart_write_IO(glc_comp_id(egi), .false., bypass_import_fields=.true., annotation="write restart file in "//trim(glc_name(egi)))
        end if
    end do
    do eri = 1, num_inst_rof
        if (iamin_ROFID(eri) .and. rof_present) then
            call CCPL_do_restart_write_IO(rof_comp_id(eri), .false., bypass_import_fields=.true., annotation="write restart file in "//trim(rof_name(eri)))
        end if
    end do
    do ewi = 1, num_inst_wav
        if (iamin_WAVID(ewi) .and. wav_present) then
            call CCPL_do_restart_write_IO(wav_comp_id(ewi), .false., bypass_import_fields=.true., annotation="write restart file in "//trim(wav_name(ewi)))
        end if
    end do
end subroutine use_ccpl_do_restart_write
#endif
